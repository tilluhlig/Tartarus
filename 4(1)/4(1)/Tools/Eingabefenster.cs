...
namespace _4_1_
{
    ...
    public static class Eingabefenster
    {
        ...
        public static void Initialisieren()
        {
            if (ErsterAufruf)
            {
                ...
                */
                ...
                Eingabe = new Textfeld(new Vector2(500 - Texturen.font3.MeasureString(("").PadLeft(30, ' ')).X / 2, Game1.screenHeight / 2 - Texturen.font3.MeasureString((" ")).Y / 2), "", 30, 10, 5, 2, Color.Gray, Texturen.font3);
            }
        }
                ...
                if (Eingabe.input.Length > 1)
                {
                    if (Eingabe.input.Substring(0, 1) == "#")
                    {
                        ...
                        if (data.Count() == 1 && text.Length >= 1)
                        {
                            ...
                            int anz = able.Count; if (anz >= 10) anz = 10;
                            int y = (int)(Eingabe.height + 2 * Eingabe.offsety);
                            int size = (int)Texturen.font2.MeasureString(" ").Y;
                            int height = (int)(anz * (size + 1) + 6);

                            if (anz > 0)
                            {
                                spriteBatch.Draw(Textfeld.textbox, new Rectangle((int)Eingabe.pos.X, (int)(Eingabe.pos.Y + y), (int)(Eingabe.width + 2 * Eingabe.offsetx), height), new Rectangle(0, 0, Textfeld.textbox.Width, Textfeld.textbox.Height), Color.DimGray);

                                for (int i = 0; i < able.Count && i < 10; i++)
                                {
                                    String aus = able[i];
                                    if (aus.Length > Eingabe.Length) aus = aus.Substring(0, Eingabe.Length - 3) + "...";
                                    spriteBatch.DrawString(Texturen.font2, aus, new Vector2((int)Eingabe.pos.X, (int)(Eingabe.pos.Y + y + 3 + (size + 1) * i)), Color.Black);
                                }
                            }
                        }
                    }
        ...
        public static void KeyboardKeys(KeyboardState keybState, KeyboardState keybState2)
        {
            ...
            if (keybState != keybState2 && !Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Enter))
            {
                if (!Eingabe.Sichtbar) return;

                if (keybState != keybState2 && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Space))
                {
                    ...
                    if (keybState != keybState2 && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Up))
                    {
                        ...
                        if (keybState != keybState2 && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Down))
                        {
                            if (hist > 0)
                            {
                                hist--;
                            ...
                            if (Eingabe.input.Substring(0, 1) == "#")
                            {
                                ...
                                if (data.Count() >= 2)
                                {
                                    ...
                                    { }
                                        ...
                                        if (Var<int>.SetFromALLE(data[0], data[1]))
                                        { }
                                        else
                                            if (Var<String>.SetFromALLE(data[0], data[1]))
                                            { }
                                            else
                                                if (Var<float>.SetFromALLE(data[0], data[1]))
                                                { }
                                                else
                                                    if (Var<bool[]>.SetFromALLE(data[0], data[1]))
                                                    { }
