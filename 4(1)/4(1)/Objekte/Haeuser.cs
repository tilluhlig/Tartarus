...
namespace _4_1_
{
    ...
    public class Haus
    {
        ...
        {
            Haus.Rucksack.Treibstoff = 9999;
            Haus.Rucksack.Munition[0] = 10;
            Haus.Rucksack.Munition[1] = 10;
            Haus.Rucksack.Munition[2] = 10;
            Haus.Rucksack.Munition[3] = 10;
            Haus.Rucksack.Munition[4] = 5;
            Haus.Rucksack.Munition[5] = 5;
            Haus.Rucksack.Munition[6] = 100;
            Haus.Rucksack.Munition[7] = 100;
            Haus.Rucksack.Munition[8] = 0;
            Haus.Rucksack.Munition[9] = 0;
            Haus.Rucksack.Munition[10] = 25;
            Haus.Rucksack.Munition[11] = 25;
            Haus.Rucksack.Munition[12] = 25;
            Haus.Rucksack.Munition[13] = 25;
            Haus.Rucksack.Munition[14] = 0;
            Haus.Rucksack.Munition[15] = 0;
            Haus.Rucksack.Munition[16] = 0;
            Haus.Rucksack.Munition[17] = 0;
            Haus.Rucksack.Munition[18] = 5000;
        }
        ...
        public bool Erobern(int HausID, int wert, int ErobererID)
        {
            ...
            if (BesitzerEroberer[HausID] == ErobererID)
            {
                ...
                if (BesitzerEroberer[HausID] != ErobererID)
                {
                    ...
                    if (BesitzerPunkte[HausID] >= Allgemein.MaxBesitzerPunkte) { BesitzerPunkte[HausID] = Allgemein.MaxBesitzerPunkte; BesitzerEroberer[HausID] = ErobererID; }
                }
        ...
        /// The HAEUSE r_ LEBENSLINIE
        /// </summary>
        private static Var<bool> MOD_HAEUSER_LEBENSLINIE = new Var<bool>("HAEUSER_LEBENSLINIE", false, ref HAEUSER_LEBENSLINIE);

        /// <summary>
        /// The HAEUSE r_ ZERSTOERUNG
        /// </summary>
        private static Var<bool> MOD_HAEUSER_ZERSTOERUNG = new Var<bool>("HAEUSER_ZERSTOERUNG", false, ref HAEUSER_ZERSTOERUNG);
        ...
        public static void generate_staedte_doerfer(List<UInt16>[] array, bool symmetrisch)
        {
            ...
            for (int i = Abstand / 2; i < breite - Abstand / 2; i++)
            {
                if (dorf)
                {
                    int[] temp = new int[Gebäudedaten.DORF.Wert.Count() + 2];
                    int[] temp2 = new int[Gebäudedaten.DORF.Wert.Count()];
                    temp[temp.Count() - 2] = i;
                ...
                {
                    int[] temp = new int[Gebäudedaten.STADT.Wert.Count() + 2];
                    int[] temp2 = new int[Gebäudedaten.STADT.Wert.Count()];
                    ...
                    for (int b = 0; b < Gebäudedaten.STADT.Wert.Count(); b++)
                    {
                        ...
                        {
                            temp[b] = Gebäudedaten.HAEUSERSTADT.Wert[rnd.Next(0, Gebäudedaten.HAEUSERSTADT.Wert.Count())];
                        }
                        else
        ...
        public void Add(Vector2 _Position, int _hp, int Typ, int _Besitzer) // fügt ein Haus ein
        {
            ...
            Color[] temp = new Color[Texturen.haus[Typ].Width * Texturen.haus[Typ].Height];
            ...
            if (HAEUSER_KOLLISION)
            {
                Kollision.Add(null);
                Kollision2.Add(null);
                Kollision2[Besitzer.Count - 1] = new KollisionsObjekt(Bild[Besitzer.Count - 1], Bild[Besitzer.Count - 1].Width, Bild[Besitzer.Count - 1].Height, Gebäudedaten.SKALIERUNG.Wert[HausTyp[Besitzer.Count - 1]], false, false, false, new Vector2(0, 0));
        ...
        {
            Kollision[i] = new KollisionsObjekt(Bild[i], Bild[i].Width, Bild[i].Height, Gebäudedaten.SKALIERUNG.Wert[HausTyp[i]], false, false, false, new Vector2(0, 0));
        }
        ...
        {
            Zerstörung[i] = new ZerstörungsObjekt(Bild[i].Width, Bild[i].Height, Gebäudedaten.SKALIERUNG.Wert[HausTyp[i]], false, false, false);
        }
        ...
        public void set_Haeuser(List<UInt16>[] Spielfeld, bool symmetrisch) // setzt zufällig Häuser auf der Karte
        {
            ...
            for (; x < (symmetrisch ? Spielfeld.Length / 2 : Spielfeld.Length); x++)
            {
                ...
                for (int b = x + 1; b < (symmetrisch ? Spielfeld.Length / 2 : Spielfeld.Length) && find < Texturen.haus[setnext].Width * Gebäudedaten.SKALIERUNG.Wert[setnext]; b++)
                {
            ...
            if (symmetrisch)
            {
                ...
                for (int i = 0; i < anz; i++)
                {
                    Add(new Vector2(middle + (middle - Position[i].X) - Bild[i].Width * Gebäudedaten.SKALIERUNG.Wert[HausTyp[i]], Position[i].Y), Lebenspunkte[i], HausTyp[i], Besitzer[i]);
                }
            }
        }
        ...
        /// <param name="Spielfeld">The spielfeld.</param>
        /// <param name="symmetrisch">if set to <c>true</c> [symmetrisch].</param>
        public void set_Haeuser_staedte_doerfer(List<UInt16>[] Spielfeld, bool symmetrisch) // setzt zufällig Häuser auf der Karte
        {
            Haus.Orte.Clear();
            ...
            for (int i = 0; i < Staedte.Count; i++)
            {
                ...
                for (int b = 0; b < Staedte[i].Count() - 2; b++)
                {
                    ...
                    float y = Kartenformat.BottomOf(x, 0) - Texturen.haus[setnext].Height * Gebäudedaten.SKALIERUNG.Wert[setnext];
                    if (y < min) min = y;
                }

                int ma = Haus.Staedte[i].Count();
                Haus.Orte.Add(new Vector2(Haus.Staedte[i][ma - 2], Haus.Staedte[i][ma - 1]));

                if (min < 25 + 25 || min > Game1.screenHeight) min = 25 + 25;
                Haus.Ortemaxheight.Add((int)min);
            }
            ...
            for (int i = 0; i < Doerfer.Count; i++)
            {
                ...
                int ma = Haus.Doerfer[i].Count();
                Haus.Orte.Add(new Vector2(Haus.Doerfer[i][ma - 2], Haus.Doerfer[i][ma - 1]));
            ...
            if (symmetrisch)
            {
                ...
                for (int i = 0; i < anz; i++)
                {
                    Haus.Orte.Add(new Vector2(middle + (middle - Haus.Orte[i].X) - Haus.Orte[i].Y, Haus.Orte[i].Y));
                    Haus.Ortemaxheight.Add(Haus.Ortemaxheight[i]);
                }
            }

            StreamReader file = new StreamReader("Content\\Konfiguration\\Orte.txt");
        ...
        public static bool ZeichneHäuser(bool found, SpriteBatch spriteBatch, GraphicsDevice graphicsDevice, Spiel Spiel2) // Zeichnet die Haeuser
        {
            ...
            for (int i = 0; i < Spiel2.Haeuser.Position.Count; i++)
            {
                ...
                if (Spiel2.Haeuser.Position[i].X + Texturen.haus[id].Width * Gebäudedaten.SKALIERUNG.Wert[id] < Spiel2.Fenster.X || Spiel2.Fenster.X + Game1.screenWidth + 10 < Spiel2.Haeuser.Position[i].X) continue;
                int xPos = (int)(Spiel2.Haeuser.Position[i].X - (int)Spiel2.Fenster.X);
                int yPos = (int)(Spiel2.Haeuser.Position[i].Y - Spiel2.Fenster.Y); // - (baum[i].Width / 2)
                ...
                if (Mod.HAEUSER_FAHNE_VISIBLE.Wert)
                {
                    ...
                    if (Spiel2.Haeuser.Besitzer[i] != -1 && Spiel2.Haeuser.BesitzerPunkte[i] >= Allgemein.MinBesitzerPunkte)
                        spriteBatch.Draw(Texturen.fahne, new Vector2(xPos + Spiel2.Haeuser.Bild[i].Width / 2 * Gebäudedaten.SKALIERUNG.Wert[id] + Gebäudedaten.POSITIONX.Wert[Spiel2.Haeuser.HausTyp[i]], yPos - Texturen.fahne.Height * scale2 + Gebäudedaten.POSITIONY.Wert[Spiel2.Haeuser.HausTyp[i]] - Spiel2.Haeuser.Bild[i].Height * Gebäudedaten.SKALIERUNG.Wert[id]), null, Spiel2.players[Spiel2.Haeuser.Besitzer[i]].Farbe, 0, new Vector2(0, 0), scale2, SpriteEffects.None, 1); // haus.Width / 2
                }
                spriteBatch.End();
                ...
                if (!Editor.visible)
                    if (Hauptfenster.Tausch.SpielAktiv)
                        if (Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
                            if (Spiel2.Haeuser.IsCollision2(i, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] + new Vector2(0, -2)))
                            {
                                treffer = true;
                            }
                ...
                if (Spiel2.Haeuser.BesitzerPunkte[i] < Allgemein.MaxBesitzerPunkte)
                {
                    ...
                    int y = (int)(float)(((float)Spiel2.Haeuser.BesitzerPunkte[i] / 1000.0f) * Texturen.haus[id].Height);
                    spriteBatch.Draw(Spiel2.Haeuser.Bild[i], new Vector2(xPos, yPos), new Rectangle(0, y, Texturen.haus[id].Width, Texturen.haus[id].Height - y), farb, 0, new Vector2(0, Texturen.haus[id].Height - y), Gebäudedaten.SKALIERUNG.Wert[id], SpriteEffects.None, 1);
                }
                ...
                if (Hauptfenster.Tausch.SpielAktiv)
                {
                    if (treffer)
                    {
                        spriteBatch.Draw(Texturen.hausumriss[id], new Vector2(xPos, yPos), null, Color.Lime, 0, new Vector2(0, Texturen.haus[id].Height), Gebäudedaten.SKALIERUNG.Wert[id], SpriteEffects.None, 1);
                    }
                    ...
                    if (!found)
                    {
                        ...
                        if (Game1.MausAktiv && Spiel2.Haeuser.HausTyp[i] == Gebäudedaten.FABRIK && Spiel2.Haeuser.Besitzer[i] == Spiel2.CurrentPlayer && Spiel2.Haeuser.BesitzerPunkte[i] >= Allgemein.MinBesitzerPunkte && Spiel2.Haeuser.Lebenspunkte[i] > 0)
                            if (Spiel2.Haeuser.IsCollision2(i, new Vector2(mouseState.X + Spiel2.Fenster.X, mouseState.Y + Spiel2.Fenster.Y) + new Vector2(0, 0)))
                            {
                                treffer3 = true;
                                found = true;
                            }
                        ...
                        if (treffer3)
                        {
                            spriteBatch.Draw(Texturen.hausumriss[id], new Vector2(xPos, yPos), null, Color.OrangeRed, 0, new Vector2(0, Texturen.haus[id].Height), Gebäudedaten.SKALIERUNG.Wert[id], SpriteEffects.None, 1);
                        }
                    }
                }
                ...
                {
                    // Häuser malen
                    spriteBatch.Draw(Texturen.haus[Spiel2.Haeuser.HausTyp[i]], new Vector2(xPos, yPos), null, Color.Blue * 255, 0, new Vector2(0, Texturen.haus[id].Height), Gebäudedaten.SKALIERUNG.Wert[id], SpriteEffects.None, 0);
                }

                // ID zeichnen
                if (Game1.DEBUG_AKTIV.Wert) spriteBatch.DrawString(Texturen.font2, Spiel2.Haeuser.HausTyp[i].ToString(), new Vector2(xPos, Spiel2.Haeuser.Position[i].Y - Texturen.haus[id].Height), Color.Yellow);
                spriteBatch.End();
            }
            return found;
        }
        ...
        public void Laden(List<String> Text, int id)
        {
            ...
            Kollision[id] = new KollisionsObjekt(Bild[id], Bild[id].Width, Bild[id].Height, Gebäudedaten.SKALIERUNG.Wert[HausTyp[id]], false, false, false, new Vector2(0, 0));
            Kollision2[id] = new KollisionsObjekt(Bild[id], Bild[id].Width, Bild[id].Height, Gebäudedaten.SKALIERUNG.Wert[HausTyp[id]], false, false, false, new Vector2(0, 0));
            Zerstörung[id] = new ZerstörungsObjekt(Bild[id].Width, Bild[id].Height, Gebäudedaten.SKALIERUNG.Wert[HausTyp[id]], false, false, false);
