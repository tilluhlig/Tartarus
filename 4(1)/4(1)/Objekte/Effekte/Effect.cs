    ...
    public class Effekt
    {
        ...
        public Effekt(Effekt A)
        {
            init(A.Name, (String) A.Bild.Tag, A.Dauer, A.Sorte, A.HP, A.HPProzent, A.MaxHP, A.MaxHPProzent,
                ...
                A.Elektrisiert, A.Tarnfaehigkeit, A.Zielhilfe, A.Faecher, A.FaecherProzent, A.VerbrauchProzent);
        }
        ...
        public Effekt(String _Name, String _Bild, int _Dauer, int _Sorte, int _HP, int _ProzentHP, int _MaxHP,
            int _ProzentMaxHP, int _Arbeitsbereich, int _ProzentArbeitsbereich, int _GeschwV, int _ProzentGeschwV,
            int _GeschwR, int _ProzentGeschwR, int _Schaden, int _ProzentSchaden, int _FeuerSchaden,
            int _FeuerProzentSchaden, int _GiftSchaden, int _GiftProzentSchaden, int _Verteidigung,
            int _ProzentVerteidigung, float _Eingefroren, float _Vergiftet, float _Elektrisiert, int _Tarnfähigkeit,
            int _Zielhilfe, int _Slots, int _ProzentSlots, int _ProzentVerbrauch)
        {
            init(_Name, _Bild, _Dauer, _Sorte, _HP, _ProzentHP, _MaxHP, _ProzentMaxHP, _Arbeitsbereich,
                _ProzentArbeitsbereich, _GeschwV, _ProzentGeschwV, _GeschwR, _ProzentGeschwR, _Schaden, _ProzentSchaden,
                _FeuerSchaden, _FeuerProzentSchaden, _GiftSchaden, _GiftProzentSchaden, _Verteidigung,
                _ProzentVerteidigung, _Eingefroren, _Vergiftet, _Elektrisiert, _Tarnfähigkeit, _Zielhilfe, _Slots,
                _ProzentSlots, _ProzentVerbrauch);
        }
        ...
        public List<String> Speichern()
        {
            ...
            data.Add("[EFFEKT]");
            data.Add("Name=" + Name);
            data.Add("Sorte=" + Sorte);
            data.Add("Dauer=" + Dauer);
            data.Add("Bild=" + (Bild == null ? "Textures//leer" : Bild.Tag));

            if (HP != 0) data.Add("HP=" + HP);
            if (HPProzent != 0) data.Add("HPProzent=" + HPProzent);
            if (MaxHP != 0) data.Add("MaxHP=" + MaxHP);
            if (MaxHPProzent != 0) data.Add("MaxHPProzent=" + MaxHPProzent);
            if (Arbeitsbereich != 0) data.Add("Arbeitsbereich=" + Arbeitsbereich);
            if (ArbeitsbereichProzent != 0) data.Add("ArbeitsbereichProzent=" + ArbeitsbereichProzent);
            if (GeschwV != 0) data.Add("GeschwV=" + GeschwV);
            if (GeschwVProzent != 0) data.Add("GeschwVProzent=" + GeschwVProzent);
            if (GeschwR != 0) data.Add("GeschwR=" + GeschwR);
            if (GeschwRProzent != 0) data.Add("GeschwRProzent=" + GeschwRProzent);
            if (Schaden != 0) data.Add("Schaden=" + Schaden);
            if (SchadenProzent != 0) data.Add("SchadenProzent=" + SchadenProzent);
            if (Verteidigung != 0) data.Add("Verteidigung=" + Verteidigung);
            if (VerteidigungProzent != 0) data.Add("VerteidigungProzent=" + VerteidigungProzent);
            if (FeuerResistenzProzent != 0) data.Add("FeuerResistenzProzent=" + FeuerResistenzProzent);
            if (GiftResistenzProzent != 0) data.Add("GiftResistenzProzent=" + GiftResistenzProzent);
            if (FeuerResistenz != 0) data.Add("FeuerResistenz=" + FeuerResistenz);
            if (GiftResistenz != 0) data.Add("GiftResistenzz=" + GiftResistenz);
            if (Eingefroren != 0) data.Add("Eingefroren=" + Eingefroren);
            if (Vergiftet != 0) data.Add("Vergiftet=" + Vergiftet);
            if (Elektrisiert != 0) data.Add("Elektrisiert=" + Elektrisiert);
            if (Tarnfaehigkeit != 0) data.Add("Tarnfaehigkeit=" + Tarnfaehigkeit);
            if (Zielhilfe != 0) data.Add("Zielhilfe=" + Zielhilfe);
            if (Faecher != 0) data.Add("Faecher=" + Faecher);
            if (FaecherProzent != 0) data.Add("FaecherProzent=" + FaecherProzent);
            if (VerbrauchProzent != 0) data.Add("VerbrauchProzent=" + VerbrauchProzent);
            data.Add("[/EFFEKT]");
        ...
        /// <param name="_ProzentSlots">The _ prozent slots.</param>
        /// <param name="_ProzentVerbrauch">The _ prozent verbrauch.</param>
        private void init(String _Name, String _Bild, int _Dauer, int _Sorte, int _HP, int _ProzentHP, int _MaxHP,
            int _ProzentMaxHP, int _Arbeitsbereich, int _ProzentArbeitsbereich, int _GeschwV, int _ProzentGeschwV,
            int _GeschwR, int _ProzentGeschwR, int _Schaden, int _ProzentSchaden, int _FeuerSchaden,
            int _FeuerProzentSchaden, int _GiftSchaden, int _GiftProzentSchaden, int _Verteidigung,
            int _ProzentVerteidigung, float _Eingefroren, float _Vergiftet, float _Elektrisiert, int _Tarnfähigkeit,
