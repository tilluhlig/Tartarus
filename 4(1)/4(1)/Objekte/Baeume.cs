...
namespace _4_1_
{
    ...
    public class Baum
    {
        ...
        /// MOD-Variable, Bäume können zerstört werden
        /// </summary>
        private static Var<bool> MOD_BAEUME_ZERSTOERUNG = new Var<bool>("BAEUME_ZERSTOERUNG", false, ref BAEUME_ZERSTOERUNG);
        ...
        public void set_Baeume(List<UInt16>[] Spielfeld, bool symmetrisch)
        {
            if (BAEUME)
            {
                ...
                for (int i = 0; ; i++)
                {
                    ...
                    Nutzloses.Hinzufügen(Texturen.baum[id], new Vector2(x, Kartenformat.BottomOf(x, 0) + 7), 0, false, Baumdata.SKALIERUNG.Wert[id], BAEUME_KOLLISION, BAEUME_ZERSTOERUNG);
                    anze++;
                }
                ...
                if (symmetrisch)
                {
                    ...
                    {
                        int id = Spiel.rand.Next(0, Texturen.baum.Length);
                        Nutzloses.Hinzufügen(Texturen.baum[id], new Vector2(middle + (middle - Nutzloses.GibPosition(max - anz + i).X), Nutzloses.GibPosition(max - anz + i).Y), 0, false, Baumdata.SKALIERUNG.Wert[id], BAEUME_KOLLISION, BAEUME_ZERSTOERUNG);
                    }
                }
            }
        }
