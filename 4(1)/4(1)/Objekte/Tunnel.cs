...
namespace _4_1_
{
    ...
    /// </summary>
        ...
        /// </summary>
        public static Var<float> TUNNEL_SCALE = new Var<float>("TUNNEL_SCALE", Optimierung.Skalierung(0.4f),
            ref SKALIERUNG);
        ...
        public static Tunnel Laden(List<String> Text, Tunnel Objekt)
        {
            ...
            {
                temp.Kollision = new KollisionsObjekt(temp.Bild, temp.Bild.Width, temp.Bild.Height, SKALIERUNG, false,
                    false, false, new Vector2(0, 0));
                temp.Zerstörung = new ZerstörungsObjekt(temp.Bild.Width, temp.Bild.Height, SKALIERUNG, false, false,
                    false);
        ...
        public static void ZeichneTunnel(SpriteBatch spriteBatch, Spiel Spiel2)
        {
            ...
            for (int i = 0; i < Spiel2.players.Count(); i++)
                for (int b = 0; b < Spiel2.players[i].TunnelAnlage.Count; b++)
                {
                    ...
                    if (tunnel[b].Position.X + Texturen.tunnel.Width*SKALIERUNG < Spiel2.Fenster.X ||
                        Spiel2.Fenster.X + Game1.screenWidth + 10 < tunnel[b].Position.X) continue;
                    var xPos = (int) (tunnel[b].Position.X - (int) Spiel2.Fenster.X);
                    var yPos = (int) (tunnel[b].Position.Y - Spiel2.Fenster.Y); // - (baum[i].Width / 2)

                    // Tunnel malen
                    if (Editor.visible && Editor.mouseover == 1 && Editor.mouseoverid == i && Editor.mouseoverid2 == b)
                    {
                        spriteBatch.Begin();
                        spriteBatch.Draw(tunnel[b].Bild, new Vector2(xPos, yPos), null, Color.Blue, 0,
                            new Vector2(0, Texturen.tunnel.Height), SKALIERUNG, SpriteEffects.None, 1);
                        spriteBatch.End();
                    }
                    else
                    {
                        ...
                        spriteBatch.Draw(tunnel[b].Bild, new Vector2(xPos, yPos), null, Color.White, 0,
                            new Vector2(0, Texturen.tunnel.Height), SKALIERUNG, SpriteEffects.None, 1);
                        spriteBatch.End();
                    }
                    ...
                    spriteBatch.Draw(Texturen.fahne,
                        new Vector2(xPos + Texturen.tunnel.Width/2*SKALIERUNG,
                            yPos - Texturen.fahne.Height*scale2 - Texturen.tunnel.Height*SKALIERUNG), null,
                        Spiel2.players[i].Farbe, 0, new Vector2(0, 0), scale2, SpriteEffects.None, 1); // haus.Width / 2

                    bool treffer = false;
                    if (Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
                        if (
                            tunnel[b].PrüfeObKollision(
                    ...
                    if (treffer)
                    {
                        spriteBatch.Draw(Texturen.tunnelumriss, new Vector2(xPos, yPos), null, Color.White, 0,
                            new Vector2(0, Texturen.tunnel.Height), SKALIERUNG, SpriteEffects.None, 1);
                    }
                    ...
                    {
                        float breite = Texturen.tunnel.Width*SKALIERUNG;

                        var leben = new Rectangle(0, 0, (int) (breite*1), Texturen.leben.Height/4);
                        spriteBatch.Draw(Texturen.leben,
                            new Vector2(xPos, tunnel[b].Position.Y - Texturen.tunnel.Height*SKALIERUNG - 10), leben,
                            Color.DarkRed*0.5f);

                        leben = new Rectangle(0, 0, (int) (breite*(tunnel[b].Lebenspunkte/Maxhp)),
        ...
        public void Lade()
        {
            Kollision = new KollisionsObjekt(Bild, Bild.Width, Bild.Height, SKALIERUNG, false, false, false,
                new Vector2(0, 0));
        }
