...
namespace _4_1_
{
    ...
    public static class Vordergrund
    {
        ...
        /// </summary>
        /// <param name="Vierecke">Ist eine Liste von Vierecken, welche zu aktualisierende
        /// Bereiche angeben.
        /// x = X-Position, z = Breite, y = Y-Position, w = Höhe</param>
            ...
            for (int c = 0; c < Spiel2.foreground.Length; c++)
            {
                ...
                for (int x = 2048 * c; x < Spiel2.Spielfeld.Length && x < 2048 * (c + 1); x++)
                {
                    ...
                    for (int y = 0; y < Spiel2.Spielfeld[x].Count; sum += Kartenformat.Laenge(Spiel2.Spielfeld[x][y]), y++)
                    {
                        ...
                        {
                            if (sum + d < Vierecke[0].Y || sum + d > Vierecke[0].Y + Vierecke[0].W) continue;  // fehlerhaft?
                            ...
                            if (Karte.Material[sorte].Farbe)
                            {
                                ...
                                t = Karte.Material[sorte].CBild[(x % Karte.Material[sorte].Bild.Width) + ((sum + d) % Karte.Material[sorte].Bild.Height) * Karte.Material[sorte].Bild.Width];
                            if (!Kartenformat.isSet(x, sum + d) && sum + d > screenHeight - 20)
                            {
                                t = water[x % Texturen.wasser.Width, (sum + d) % Texturen.wasser.Height];
                            }
                            else
                                if ((!Kartenformat.isSet(x, sum + d - 5) || !Kartenformat.isSet(x + 5, sum + d) || !Kartenformat.isSet(x, sum + d + 5) || !Kartenformat.isSet(x - 5, sum + d)) && Karte.Material[sorte].Abdunkeln)
                                {
                                    t.R /= (int)2f; t.G /= (int)2f; t.B /= (int)2f;
                                }
        ...
        /// </summary>
        /// <param name="Bereiche">Ist eine Liste von zu aktualisierenden Bereichen.
        /// x = X-Position, y = Y-Begin, z = Y-Ende</param>
        ...
        {   // Übergabe:  x , y , Breite, Höhe
            ...
            Color[,] water = Game1.Farbwahl(Texturen.wasser);
            bool[] ischanged = new bool[Spiel2.foreground.Count()];
            ...
            for (int i = 0; i < Bereiche.Count; i++)
            {
                ...
                if (Bereiche[i].Z >= screenHeight) Bereiche[i] = new Vector3(Bereiche[i].X, Bereiche[i].Y, screenHeight - 1);

                int c = (int)(Bereiche[i].X / Bildbreite);
                ischanged[c] = true;
                int x = (int)Bereiche[i].X;
                int h = (x - c * 2048);
                int sum = 0;
                for (int y = 0; y < Spiel2.Spielfeld[x].Count; sum += Kartenformat.Laenge(Spiel2.Spielfeld[x][y]), y++)
                {
                    ...
                    for (int d = 0; d < anzahl; d++)
                    {
                        ...
                        if (!Kartenformat.isSet(x, sum + d) && sum + d > screenHeight - 20)
                        {
                            ...
                            if (!Editor.visible && (!Kartenformat.isSet(x, sum + d - 5) || !Kartenformat.isSet(x + 5, sum + d) || !Kartenformat.isSet(x, sum + d + 5) || !Kartenformat.isSet(x - 5, sum + d)) && Karte.Material[sorte].Abdunkeln)
                            {
                                t.R /= (int)2f; t.G /= (int)2f; t.B /= (int)2f;
                            }
                        Spiel2.foregroundColors[c][h + (sum + d) * Bildbreite] = t;
