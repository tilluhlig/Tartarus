...
namespace _4_1_
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        /// The line
        /// </summary>
        private static Texture2D line = null;
        ...
        /// The smoke list
        /// </summary>
        private List<Karte.ParticleData> smokeList = new List<Karte.ParticleData>();
        ...
        /// The XWERT
        /// </summary>
        private int XWERT = Hauptfenster.Tausch.screenwidth;

        /// <summary>
        /// The YWERT
        /// </summary>
        private int YWERT = Hauptfenster.Tausch.screenheight;
        ...
        public static void LadeText(String Text2)
        {
            if (Ladebildschirmtexte == null || Ladebildschirmtexte.Count == 0)
            {
                if (File.Exists("Content\\Konfiguration\\Ladetexte.txt"))
                {
                    ...
                    ReaderStream.ReaderStream b = new ReaderStream.ReaderStream("Content\\Konfiguration\\Ladetexte.txt");
            ...
            Hauptfenster.Program.Formular.label31.Text = ("").PadRight((int)(((float)Hauptfenster.Program.Formular.progressBar1.Width / 5.4f) + 0.5f), ' ');
            ...
            Hauptfenster.Program.Formular.label31.Top = Hauptfenster.Program.Formular.progressBar1.Top - Hauptfenster.Program.Formular.label31.Height; // + progressBar1.Height
            Hauptfenster.Program.Formular.label31.Left = Hauptfenster.Program.Formular.Width / 2 - Hauptfenster.Program.Formular.label31.Width / 2;
        ...
        {
            if (Hauptfenster.Tausch.Output.Count > 0)
            {
                for (int i = 0; i < Hauptfenster.Tausch.Output.Count; i++)
                {
                    String text = Hauptfenster.Tausch.Output[i];
                    if (Spiel2.IsServer)
                    {
                        Server.Send("<>" + text);
                    }
                ...
                Hauptfenster.Tausch.Output.Clear();
            }
            ...
            baseScreenSize = new Vector2(Hauptfenster.Tausch.screenwidth, Hauptfenster.Tausch.screenheight);

            screenWidth = (int)baseScreenSize.X;
            screenHeight = (int)baseScreenSize.Y;
            Kartengroesse = (int)(2048 * Hauptfenster.Tausch.Kartengroesse);
            SpielAktiv = Hauptfenster.Tausch.SpielAktiv;
            if (Spiel2 != null)
            {
                Spiel2.CurrentPlayer = Hauptfenster.Tausch.CurrentPlayer;
            }

            if (Hauptfenster.Tausch.SpielLaden)
            {
                ...
                if (Hauptfenster.Tausch.CreateNewGame)
                {
                    // Neues Spiel erstellen und hochladen
                    Hauptfenster.Tausch.CreateNewGame = false;
                    ...
                    // Mod laden
                    Hauptfenster.Tausch.Mod = "A.conf";
                    ...
                    if (Hauptfenster.Tausch.StartServer)
                    {
                        if (Server.isRunning) Server.Shutdown();
                        if (Client.isRunning) Client.Shutdown();
                        Hauptfenster.Tausch.StartServer = false;
                        StarteNeuesSpiel();
                        Server.StartServer();
                        ...
                        if (Hauptfenster.Tausch.StartClient)
                        {
                            if (Server.isRunning) Server.Shutdown();
                            if (Client.isRunning) Client.Shutdown();
                            Hauptfenster.Tausch.StartClient = false;
                            //
                            StarteNeuesSpiel();
                            //
                            Client.Connect(Hauptfenster.Tausch.ZielIP, 14242);
                            Client.Spiel2 = Spiel2;
                            Client.game = this;
                            Spiel2.IsServer = false;
                        }
                        else
                            if (Hauptfenster.Tausch.StarteSpiel)
                            {
                                Spiel2 = null;
                                Hauptfenster.Tausch.StarteSpiel = false;
                                StarteNeuesSpiel();
        ...
        public void DrawAirstrike()
        {
            if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == 5)
            {
                ...
                int yPos = (int)screenHeight / 2 - (int)Spiel2.Fenster.Y;
                spriteBatch.Draw(Texturen.kreuz, new Vector2(xPos, Kartenformat.BottomOf(Spiel2.players[Spiel2.CurrentPlayer].shootingPower, 0)), null, Color.White, 0, new Vector2(35, 35), 1f, SpriteEffects.None, 1);
        ...
        public void DrawArbeitsbereich()
        {
            ...
            int x1 = (int)(oldpos.X - Fahrzeugdaten.ARBEITSBEREICH.Wert[id] - Spiel2.Fenster.X);
            int x2 = (int)(oldpos.X + Fahrzeugdaten.ARBEITSBEREICH.Wert[id] - Spiel2.Fenster.X);
            int size = 40;
            if (x1 >= 0 && x1 < Spiel2.Width)
            {
                Help.DrawLine(spriteBatch, new Vector2(x1 + size / 2, 0), new Vector2(x1 + size / 2, Spiel2.Height), Color.Goldenrod * 0.2f, size);
            }
            if (x2 >= 0 && x2 < Spiel2.Width)
            {
                Help.DrawLine(spriteBatch, new Vector2(x2 + size / 2, 0), new Vector2(x2 + size / 2, Spiel2.Height), Color.Goldenrod * 0.2f, size);
            }
        }
        ...
        public void DrawBunker()
        {
            ...
            {
                int check = -1;
                int breite = (int)(Texturen.bunker[0].Width * scale);
                ...
                int xPos = (int)(Spiel2.Bunker.Position[i].X - (int)Spiel2.Fenster.X);
                int yPos = (int)(Spiel2.Bunker.Position[i].Y - Spiel2.Fenster.Y);
                ...
                if (check == -1 || check != Spiel2.CurrentPlayer)
                {
                    spriteBatch.Draw(Texturen.bunker[Bild], new Vector2(xPos, yPos), null, Color.White, 0, new Vector2(0, Texturen.bunker[Bild].Height), scale, SpriteEffects.None, 1); // haus.Width / 2
                }
                else
                    spriteBatch.Draw(Texturen.bunker2[Bild], new Vector2(xPos, yPos), null, Color.White, 0, new Vector2(0, Texturen.bunker[Bild].Height), scale, SpriteEffects.None, 1); // haus.Width / 2
                spriteBatch.End();
                ...
                if (Editor.visible && Editor.mouseover == 4 && Editor.mouseoverid == i)
                {
                    if (check == -1 || check != Spiel2.CurrentPlayer)
                    {
                        spriteBatch.Draw(Texturen.bunker[Bild], new Vector2(xPos, yPos), null, Color.Blue, 0, new Vector2(0, Texturen.bunker[Bild].Height), scale, SpriteEffects.None, 1); // haus.Width / 2
                    }
                    else
                        spriteBatch.Draw(Texturen.bunker2[Bild], new Vector2(xPos, yPos), null, Color.Blue, 0, new Vector2(0, Texturen.bunker[Bild].Height), scale, SpriteEffects.None, 1); // haus.Width / 2
                } spriteBatch.End();
                ...
                if (Bunker.BUNKER_LEBENSLINIE)
                {
                    ...
                    spriteBatch.Draw(Texturen.leben, new Vector2(xPos, Spiel2.Bunker.Position[i].Y - Texturen.bunker[Bild].Height * scale - 10), leben,
                           Color.DarkRed * 0.5f);

                    if (Spiel2.Bunker.Lebenspunkte[i] > 0)
                    {
                        leben = new Rectangle(0, 0, (int)(Breite * ((float)Spiel2.Bunker.Lebenspunkte[i] / (Spiel2.Bunker.MaximaleLebenspunkte[i]))), Texturen.leben.Height / 4);
                        spriteBatch.Draw(Texturen.leben, new Vector2(xPos, Spiel2.Bunker.Position[i].Y - Texturen.bunker[Bild].Height * scale - 10), leben,
                               Color.Lime * 0.5f);
                    }
                } spriteBatch.End();
        ...
        public void DrawExplosion()
        {
            for (int i = 0; i < Spiel2.Karte.particleListExp.Count; i++)
            {
                ...
                spriteBatch.Draw(Texturen.explosion, new Vector2(xPos, yPos), null, particle.ModColor, i, new Vector2(256, 256), particle.Scaling, SpriteEffects.None, 1);
            }
        }
        ...
        public void DrawKisten()
        {
            ...
            for (int b = 0; b < Spiel2.Kisten.aktiv.Count; b++)
            {
                ...
                int x = (int)Spiel2.Kisten.pos[b].X;
                int y = (int)Spiel2.Kisten.pos[b].Y;

                Texture2D Bild = Kiste.Bild;
                if (x == -1) continue;
                if (x + Bild.Width * scale < Spiel2.Fenster.X || Spiel2.Fenster.X + screenWidth < x - Bild.Width * scale) continue;
                int xPos = (int)(x - (int)Spiel2.Fenster.X);
                int yPos = (int)(y - Spiel2.Fenster.Y);

                spriteBatch.Draw(Bild, new Vector2(xPos - (Bild.Width * scale) / 2, yPos - (Bild.Height * scale) / 2), null, Color.White, 0, new Vector2(Bild.Width / 2, Bild.Height / 2), scale, SpriteEffects.None, 1);

                if (Editor.visible && Editor.mouseover == 5 && Editor.mouseoverid == b)
                    spriteBatch.Draw(Bild, new Vector2(xPos - (Bild.Width * scale) / 2, yPos - (Bild.Height * scale) / 2), null, Color.Blue, 0, new Vector2(Bild.Width / 2, Bild.Height / 2), scale, SpriteEffects.None, 1);
            }
        }
        ...
        public void DrawLeiste()
        {
            if (Spiel2.CurrentPlayer != -1)
                if (Spiel2.players[Spiel2.CurrentPlayer].CurrentTank != -1)
                {
                    ...
                    {
                        String fuel = "";
                        if (Spieler.GLOBAL_FUEL.Wert) fuel = Spiel2.players[Spiel2.CurrentPlayer].fuelRemains < 0 ? "0000" : ((Math.Floor(Spiel2.players[Spiel2.CurrentPlayer].fuelRemains > 9999 ? 9999 : Spiel2.players[Spiel2.CurrentPlayer].fuelRemains)).ToString()).PadLeft(4, '0');
                        if (Spieler.PLAYER_FUEL.Wert) fuel = Spiel2.players[Spiel2.CurrentPlayer].Rucksack[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].GibTreibstoff() < 0 ? "0000" : ((Math.Floor(Spiel2.players[Spiel2.CurrentPlayer].Rucksack[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].GibTreibstoff() > 9999 ? 9999 : Spiel2.players[Spiel2.CurrentPlayer].Rucksack[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].GibTreibstoff())).ToString()).PadLeft(4, '0');

                        Vector2 add = Spiel.CREDITS.Wert ? new Vector2(-((float)Texturen.LeeresFeld.Width * scale5) - 25, 0) : Vector2.Zero;

                        if (Mod.ACTION_BUTTON_VISIBLE.Wert)
                        {
                            spriteBatch.Draw(Texturen.LeeresFeld, new Vector2(screenWidth - 3 - 2 * ((float)Texturen.LeeresFeld.Width * scale5), 5) + add, null, Color.White, 0, new Vector2(0, 0), scale5, SpriteEffects.None, 1);
                            spriteBatch.DrawString(Texturen.font, fuel, new Vector2(screenWidth - 3 - 2 * ((float)Texturen.LeeresFeld.Width * scale5) + 8, 5 + 10) + add, Color.Green);
                        }
                        else
                        {
                            spriteBatch.Draw(Texturen.LeeresFeld, new Vector2(screenWidth - 3 - ((float)Texturen.LeeresFeld.Width * scale5), 5) + add, null, Color.White, 0, new Vector2(0, 0), scale5, SpriteEffects.None, 1);
                            spriteBatch.DrawString(Texturen.font, fuel, new Vector2(screenWidth - 3 - ((float)Texturen.LeeresFeld.Width * scale5) + 8, 5 + 10) + add, Color.Green);
                        }
                    }
                    ...
                    {
                        String credits = (((int)Spiel2.players[Spiel2.CurrentPlayer].Credits > 9999 ? 9999 : Spiel2.players[Spiel2.CurrentPlayer].Credits).ToString()).PadLeft(4, '0');
                        spriteBatch.Draw(Texturen.LeeresFeld, new Vector2(screenWidth - 3 - ((float)Texturen.LeeresFeld.Width * scale5) * 2, 5), null, Color.White, 0, new Vector2(0, 0), scale5, SpriteEffects.None, 1);
                        spriteBatch.DrawString(Texturen.font, credits, new Vector2(screenWidth - 3 - ((float)Texturen.LeeresFeld.Width * scale5) * 2 + 8, 5 + 10), Color.CadetBlue);
                    }

                    // Action Points Button
                    if (Mod.ACTION_BUTTON_VISIBLE.Wert)
                    {
                        String action = (((int)Spiel2.players[Spiel2.CurrentPlayer].ActionPoints > 9999 ? 9999 : Spiel2.players[Spiel2.CurrentPlayer].ActionPoints).ToString()).PadLeft(4, '0');
                        spriteBatch.Draw(Texturen.LeeresFeld, new Vector2(screenWidth - 3 - ((float)Texturen.LeeresFeld.Width * scale5), 5), null, Color.White, 0, new Vector2(0, 0), scale5, SpriteEffects.None, 1);
                        spriteBatch.DrawString(Texturen.font, action, new Vector2(screenWidth - 3 - ((float)Texturen.LeeresFeld.Width * scale5) + 8, 5 + 10), Color.Red);
                    }
                    ...
                    if (Mod.AKTUELLE_MUNITION_BUTTON_VISIBLE.Wert)
                    {
                        ...
                        cool = cool.PadLeft(5, ' ');
                        spriteBatch.DrawString(Texturen.font2, cool, new Vector2(Mod.AKTUELLE_MUNITION_BUTTON_X.Wert + Texturen.waffenbilder[id].Width * scale - Texturen.font2.MeasureString(cool).X - 5, Mod.AKTUELLE_MUNITION_BUTTON_Y.Wert + Texturen.waffenbilder[id].Height * scale - Texturen.font2.MeasureString(cool).Y - 5), Color.Red * 0.8f);

                        // Munitionsvorrat
                        if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 0 && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 5)
                        {
                            cool = Convert.ToString(Spiel2.players[Spiel2.CurrentPlayer].Munition[tank][id]);
                            //   cool = cool.PadLeft(5, ' ');
                            spriteBatch.DrawString(Texturen.font2, cool, new Vector2(Mod.AKTUELLE_MUNITION_BUTTON_X.Wert + 10, Mod.AKTUELLE_MUNITION_BUTTON_Y.Wert + 5), Color.PaleGoldenrod);
                        }
                    }
                    ...
                    else
                    {
                        if (curr < 0)
                        {
                            ...
                            if (curr + leistenbreite >= list.Count)
                            {
                    ...
                    for (int i = 0; i < list.Count && i < leistenbreite; i++)
                    {
                        ...
                        if (Mod.LEISTE_BUTTON_VISIBLE.Wert)
                        {
                            ...
                            r = Color.Gold;
                            r *= 0.8f;
                            spriteBatch.Draw(Texturen.panzerbutton[current.KindofTank[b]], new Vector2(5 + ((float)Texturen.LeeresFeld.Width * scale) * i, screenHeight - (float)Texturen.LeeresFeld.Height * scale - 5), null, r, 0, new Vector2(0, 0), scale, SpriteEffects.None, 1);
                        }
                        ...
                        if (Spieler.WAFFEN_COOLDOWN.Wert)
                        {
                            ...
                            spriteBatch.DrawString(Texturen.font2, Convert.ToString(cool / 60), new Vector2(5 + ((float)Texturen.LeeresFeld.Width * scale) * i + 10, screenHeight - (float)Texturen.LeeresFeld.Height * scale - 5 + 31), rr);
                        }
                        ...
                        if (Mod.LEISTE_LEBENSLINIE_VISIBLE.Wert)
                        {
                            ...
                            int ges = (int)(((float)anteil / Fahrzeugdaten.MAXHP.Wert[current.KindofTank[b]]) * 250);
                            Rectangle a = new Rectangle(0, 0, ges, Texturen.leben.Height);
                            spriteBatch.Draw(Texturen.leben, new Vector2(10 + ((float)Texturen.LeeresFeld.Width * scale) * i, screenHeight - (float)Texturen.LeeresFeld.Height * scale - 5 + 5), a, Color.Lime * 0.5f, 0, new Vector2(0, 0), 0.25f, SpriteEffects.FlipHorizontally, 1);
                        }
                    }
                    ...
                    if (Mod.LEISTE_AKTUELL_VISIBLE.Wert)
                    {
                        ...
                        spriteBatch.Draw(Texturen.pfeil, new Vector2(5 + ((float)Texturen.LeeresFeld.Width * scale) * (curr2 - curr) + ((float)Texturen.LeeresFeld.Width * scale) / 2 - ((float)Texturen.pfeil.Width * scale2) / 2, (float)screenHeight - Texturen.LeeresFeld.Height * scale - 5 - Texturen.pfeil.Height * scale2), null, Color.White, 0, new Vector2(0, 0), scale2, SpriteEffects.None, 1);
                    }
                }
        }
        ...
        public void DrawMinen()
        {
            ...
            for (int b = 0; b < Spiel2.players[i].Minen.Count; b++)
            {
                ...
                int x = (int)Spiel2.players[i].Minen[b].Position.X;
                int y = (int)Spiel2.players[i].Minen[b].Position.Y;
                ...
                spriteBatch.Draw(Bild, new Vector2(xPos - (Bild.Width * scale) / 2, yPos), null, Color.White, 0, new Vector2(0, Bild.Height), scale, SpriteEffects.None, 1);

                if (Editor.visible && Editor.mouseover == 6 && Editor.mouseoverid == i && Editor.mouseoverid2 == b)
                    spriteBatch.Draw(Bild, new Vector2(xPos - (Bild.Width * scale) / 2, yPos), null, Color.Blue, 0, new Vector2(0, Bild.Height), scale, SpriteEffects.None, 1);
            }
        }
        ...
        public void DrawMinenKreis()
        {
            ...
            for (int b = 0; b < Spiel2.players[i].Minen.Count; b++)
            {
                ...
                int Energie = (int)Waffendaten.Daten[Spiel2.players[i].Minen[b].Waffenart].X;
                if (x == -1) continue;
                if (x + Bild.Width * scale < Spiel2.Fenster.X || Spiel2.Fenster.X + screenWidth < x - Bild.Width * scale) continue;
                int xPos = (int)(x - (int)Spiel2.Fenster.X);
                int yPos = (int)(y - Spiel2.Fenster.Y);
                ...
                {
                    faktor = (float)Spiel2.players[i].Minen[b].RadiusAnzeige / 300.0f;
                }

                spriteBatch.Draw(Bild, new Rectangle(xPos - Energie, yPos - Energie, Energie * 2, Energie * 2), null, Color.Yellow * 0.75f * faktor, 0, Vector2.Zero, SpriteEffects.None, 1);
            }
        }
        ...
        public void DrawMinimap()
        {
            ...
            int fensterx = (int)(Spiel2.Fenster.X - screenWidth2 / 2 + screenWidth / 2);
            float fact = 0.15f;

            for (int i = 0; i < Spiel2.foreground.Length; i++)
            {
                if (fensterx > (i + 1) * 2048 || fensterx + screenWidth2 < i * 2048) continue;
                if ((int)fensterx + screenWidth2 < i * 2048) continue;
                int x = (int)fensterx - i * 2048;
                ...
                if (x < 0)
                {
                    x = (i * 2048 - (int)fensterx);
                    a = new Rectangle(0, y, 2048, (int)(screenHeight)); //screenWidth - screenWidth2 * fact
                    //
                    spriteBatch.Draw(Spiel2.foreground[i], new Vector2(screenWidth - screenWidth2 * fact, screenHeight - screenHeight * fact), a, col, 0.0f, new Vector2(-x, 0), fact, SpriteEffects.None, 1);
                }
                else
                {
                    int l = 2048 - x; if (l > 2048) l = 2048; if (l < 0) l = 0;
                    a = new Rectangle(x, y, l, screenHeight);
                    // - screenHeight * fact
                    spriteBatch.Draw(Spiel2.foreground[i], new Vector2(screenWidth - screenWidth2 * fact, screenHeight - screenHeight * fact), a, col, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
                }
            }

            Rectangle c = new Rectangle(0, 0, screenWidth, (int)(screenHeight));
            // - screenHeight * fact
            spriteBatch.Draw(Texturen.kasten, new Vector2(screenWidth - screenWidth2 / 2 * fact - screenWidth / 2 * fact, screenHeight - screenHeight * fact), c, Color.White, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
        }
        ...
        public void DrawMinimapDot()
        {
            ...
            for (int b = 0; b < Spiel2.players[i].pos.Count; b++)
            {
                ...
                int x = (int)(Spiel2.players[i].pos[b].X - Texturen.dot.Width / 2 - fensterx);
                int y = (int)(Spiel2.players[i].pos[b].Y - Texturen.dot.Height / 2 - Spiel2.Fenster.Y);
                spriteBatch.Draw(Texturen.dot, new Rectangle((int)(screenWidth - (float)screenWidth2 * fact + x * fact - 1), (int)(screenHeight - screenHeight * fact + y * fact - 1), (int)(Texturen.dot.Width * fact + 3), (int)(Texturen.dot.Height * fact + 3)), new Rectangle(0, 0, 10, 10), Color.Black);

                spriteBatch.Draw(Texturen.dot, new Vector2(screenWidth - (float)screenWidth2 * fact + x * fact, screenHeight - screenHeight * fact + y * fact), null, Spiel2.players[i].Farbe, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
            }
        }
        ...
        public void DrawMissle()
        {
            ...
            for (int i = 0; i < Spiel2.Missile.Length; i++)
            {
                ...
                {
                    if (Spiel2.Missile[i].Besitzer[0] != 3 && Spiel2.Missile[i].Besitzer[0] >= 0) // Spieler 3 ist nobody
                    {
                        farbe = Spiel2.players[Spiel2.Missile[i].Besitzer[0]].Farbe;
                    }
                }

                spriteBatch.Draw(Texturen.missle[Spiel2.Missile[i].Art], new Vector2(xPos, yPos), null, farbe, Spiel2.Missile[i].missleAngle,
                        new Vector2(0, Texturen.missle[Spiel2.Missile[i].Art].Height / 2) * Waffendaten.Skalierung[Spiel2.Missile[i].Art], Waffendaten.Skalierung[Spiel2.Missile[i].Art], SpriteEffects.None, 1); // 190, 40
                ...
                if (Spiel2.Missile[i].Art == 5) //  || Spiel2.Missile[i].Art == 6 || Spiel2.Missile[i].Art == 7
                {
                    ...
                    spriteBatch.Draw(Texturen.kreuz, new Vector2(xPos, (int)(Kartenformat.BottomOf(Spiel2.Missile[i].misslePosition) - Spiel2.Fenster.Y)), null, Color.Blue, 0, new Vector2(35 * 0.5f, 35 * 0.5f), 0.5f, SpriteEffects.None, 1);
                }
                ...
                if (Mod.MISSILE_STRICH_VISIBLE.Wert)
                    ...
                    {
                        xPos = (int)Spiel2.Missile[i].misslePosition.X - (int)Spiel2.Fenster.X;
                        spriteBatch.Draw(Texturen.strich, new Vector2(xPos, 0), null, Color.White, 0, new Vector2(3, 0), 0.5f, SpriteEffects.None, 1);
                    }
            }
        }
            ...
            for (int i = 0; i < Haus.Orte.Count; i++)
            {
              ...
              new Vector2(0, 0), 1f, SpriteEffects.None, 0);
                ...
                try
                {
                    ...
                    spriteBatch.DrawString(Texturen.font2, Name, new Vector2(posx - size.X / 2, posy - size.Y / 2 + 14), Color.Black * 0.7f);
                }
                catch (Exception)
                {
                    // nichts
        ...
        public bool DrawPlayers(bool found)
        {
            ...
            for (int i = 0; i < Spiel2.players.Length; i++)
            {
                for (int b = 0; b < Spiel2.players[i].pos.Count; b++)
                {
                    ...
                    {
                        spriteBatch.Begin();
                        if (Spiel2.CurrentPlayer == i && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank == b && Spiel2.players[Spiel2.CurrentPlayer].Effekte[b].GibZielhilfe() && Spiel2.players[Spiel2.CurrentPlayer].shootingPower >= 2 && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 1)
                        {
                            // Zeichne Flugbahn
                            ...
                            c *= Spiel2.players[Spiel2.CurrentPlayer].shootingPower / (float)Math.Log(Spiel2.players[Spiel2.CurrentPlayer].shootingPower, Math.E);
                            getBahn(Waffen.gravity / 10.0f + (Spiel.WIND.Wert ? Spiel2.Wind / 60.0f : Vector2.Zero), c, a, screenHeight, (int)(a2.X - 1000), (int)(a2.X + 1000), spriteBatch);
                        }
                        ...
                        int xPos2 = (int)(Math.Sin(Spiel2.players[i].vehikleAngle[b]) * -25);
                        ...
                        if (Mod.LEBENSLINIE_ON_TANK_VISIBLE.Wert)
                        {
                            ...
                            {
                                Rectangle a = new Rectangle(0, 0, 300, Texturen.leben.Height);
                                Vector2 p = Help.RotatePositionOffset(new Vector2(xPos, yPos), Spiel2.players[i].vehikleAngle[b], new Vector2(75 / 2, Texturen.panzerindex[Spiel2.players[i].KindofTank[b]].Height * Fahrzeugdaten.SCALEP.Wert[Spiel2.players[i].KindofTank[b]] + 10));
                             ...
                             Vector2.Zero, 0.25f, SpriteEffects.FlipHorizontally, 1);
                                ...
                                int max = Spiel2.players[i].Effekte[b].GetMaxHP(Fahrzeugdaten.MAXHP.Wert[Spiel2.players[i].KindofTank[b]]);
                                if (anteil > max) anteil = max;
                                int ges = (int)(((float)anteil / max) * 300);
                                a = new Rectangle(0, 0, ges, Texturen.leben.Height);

                                spriteBatch.Draw(Texturen.leben, p, a, Color.Lime * 0.5f, Spiel2.players[i].vehikleAngle[b],
                             Vector2.Zero, 0.25f, SpriteEffects.FlipHorizontally, 1);
                            }
                        }

                        // Erfahrungspunktelinie malen
                        if (Spiel2.CurrentPlayer == i && Spiel2.players[i].ExpNow[b] > 0 && Spiel2.players[i].CurrentLv[b] < 3)
                        {
                            Rectangle a = new Rectangle(0, 0, 300, Texturen.leben.Height);
                            Vector2 p = Help.RotatePositionOffset(new Vector2(xPos, yPos + Texturen.leben.Height * 0.25f), Spiel2.players[i].vehikleAngle[b], new Vector2(75 / 2, Texturen.panzerindex[Spiel2.players[i].KindofTank[b]].Height * Fahrzeugdaten.SCALEP.Wert[Spiel2.players[i].KindofTank[b]] + 10));
                        ...
                        if (Editor.visible && Editor.mouseover == 3 && Editor.mouseoverid == i && Editor.mouseoverid2 == b)
                        {
                            Spiel2.players[i].DrawPlayer(b, spriteBatch, Spiel2.players[i].Angle[b], Spiel2.players[i].vehikleAngle[b], Fahrzeugdaten.SCALEP.Wert[Spiel2.players[i].KindofTank[b]], Fahrzeugdaten.SCALER.Wert[Spiel2.players[i].KindofTank[b]], Spiel2.players[i].pos[b], Spiel2.Fenster, Spiel2.players[i].overreach[b], true, 1.0f, 1.0f, Color.Blue, Besitzer);
                        }
                        else
                            Spiel2.players[i].DrawPlayer(b, spriteBatch, Spiel2.players[i].Angle[b], Spiel2.players[i].vehikleAngle[b], Fahrzeugdaten.SCALEP.Wert[Spiel2.players[i].KindofTank[b]], Fahrzeugdaten.SCALER.Wert[Spiel2.players[i].KindofTank[b]], Spiel2.players[i].pos[b], Spiel2.Fenster, Spiel2.players[i].overreach[b], (i == Spiel2.CurrentPlayer && b == Spiel2.players[Spiel2.CurrentPlayer].CurrentTank && !Editor.visible) ? true : false, 1.0f, 1.0f, Color.DarkGoldenrod, Besitzer);
                        ...
                        if (!Editor.visible)
                            if (!found && i == Spiel2.CurrentPlayer)
                            {
                                if (Hauptfenster.Tausch.SpielAktiv)
                                    if (Spiel2.players[i].PrüfeObKollision(b, new Vector2(mouseState.X + Spiel2.Fenster.X, mouseState.Y + Spiel2.Fenster.Y)))
                                    {
                                        int Kindof = Spiel2.players[i].KindofTank[b];
                                        spriteBatch.Draw(Texturen.panzerumriss[Kindof], new Vector2(xPos, yPos), null, Color.OrangeRed, Spiel2.players[i].vehikleAngle[b],
                                         new Vector2(Texturen.panzerumriss[Kindof].Width / 2, Texturen.panzerumriss[Kindof].Height), Fahrzeugdaten.SCALEP.Wert[Kindof], Spiel2.players[i].overreach[b] ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
                                        found = true;
                                    }
                            }

                        // shootingpower
                        if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 4 && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 5)
                            if (Spiel2.players[i].CurrentWeapon != 5 && Spiel2.players[i].CurrentTank == b && Spiel2.CurrentPlayer == i && !Spiel2.increaseairstrike && Spiel2.players[i].shootingPower > 2)
                            {
                                /*  int dicke = 4;
                                  int c = b;
                ...
                */
                        ...
                        if (Besitzer && Mod.NAME_TANK_VISIBLE.Wert)
                        {
                            try
                            {
                                ...
                                Vector2 add = new Vector2(temppos.X - Texturen.font2.MeasureString(Spiel2.players[i].Namen[b]).X / 2, (temppos.Y < 50 ? 50 : temppos.Y));
                                Help.DrawString(spriteBatch, Texturen.font2, Spiel2.players[i].Namen[b], add, Spiel2.players[i].Farbe, Color.Black);
            ...
            for (int i = 0; i < Spiel2.players.Length; i++)
            {
                for (int b = 0; b < Spiel2.players[i].pos.Count; b++)
                {
                    ...
                    if (Spiel2.players[i].isthere[b] && cool > 0)
                    {
                        ...
                        if (Spiel2.players[i].Effekte[b].GetEingefroren()) { r = Color.Aquamarine; }
                        float correkt = Texturen.panzerindex[Spiel2.players[i].KindofTank[b]].Height * Spiel2.players[i].Size[b];
                        spriteBatch.DrawString(Texturen.font, Convert.ToString(Math.Ceiling((double)cool / 60)), new Vector2(xPos, yPos - correkt), r);
                    }
                }
            }
        }
        ...
        public void DrawSmoke(GameTime gameTime)
        {
            for (int i = 0; i < smokeList.Count; i++)
            {
                ...
                int xPos = (int)particle.Position.X - (int)Spiel2.Fenster.X;
                int yPos = (int)particle.Position.Y - (int)Spiel2.Fenster.Y;
                spriteBatch.Draw(Texturen.smokeTexture, new Vector2(xPos, yPos), null, particle.ModColor, i, new Vector2(256, 256), particle.Scaling, SpriteEffects.None, 1);
            }
        ...
        public void DrawText()
        {
            ...
            if (Spiel.TIMEOUT.Wert)
            {
                ...
                spriteBatch.DrawString(Texturen.font, Spiel2.Timeout > 0 ? t : "---", new Vector2(25, screenHeight - 35), Color.Gold);
            }
        ...
        public void DrawTextEnd()
        {
            ...
            spriteBatch.DrawString(Texturen.font, bez[Gewinner] + " hat gewonnen...", new Vector2(screenWidth / 2 - 100, 100), Color.Gold);
        ...
        public void DrawWind()
        {
            ...
            if (Spiel2.Wind.X != 0)
            {
                anteil = (int)((double)((double)Texturen.wind.Width / 120) * Spiel2.Wind.X * 20);
            }
            else
                anteil = 0;
            if (anteil < 0) { anteil = -anteil; flip = true; }
            anteil = Texturen.wind.Width - anteil;
            Rectangle a;
            a = new Rectangle(anteil, 0, Texturen.wind.Width - anteil, Texturen.wind.Height);

            int xPos = (int)(-Texturen.wind.Width * Optimierung.Skalierung(0.5f));
            int yPos = 65 - 15;
            if (flip)
            {
                xPos = (int)((double)screenWidth + 2 * xPos + (Texturen.wind.Width * 0.5f / 120) * ((double)120 - ((double)-Spiel2.Wind.X * 20)));

                // TODO Origin geändert von new Vector2(35,35) auf new Vector2(0,0), richtig?
                spriteBatch.Draw(Texturen.wind, new Vector2(xPos, yPos), a, Color.Green, 0, new Vector2(0, 0), Optimierung.Skalierung(0.5f), SpriteEffects.FlipHorizontally, 1);
            }
            else
            {
                xPos = screenWidth + xPos;
                // TODO Origin geändert von new Vector2(35,35) auf new Vector2(0,0), richtig?
                spriteBatch.Draw(Texturen.wind, new Vector2(xPos, yPos), a, Color.Green, 0, new Vector2(0, 0), Optimierung.Skalierung(0.5f), SpriteEffects.None, 1);
            }
        }
        ...
        public void loadAllContent()
        {
            ...
            if (spriteBatch == null)
            {
                Hauptfenster.Program.Formular.tabControl1.KeyPress += new KeyPressEventHandler(this.OnKeyPress);
            ...
            screenWidth = Hauptfenster.Tausch.screenwidth;// (int)baseScreenSize.X;
            screenHeight = Hauptfenster.Tausch.screenheight;// (int)baseScreenSize.Y;
            ...
            if (Mod.PAUSEMENU_VISIBLE.Wert) pauseMenu = new Menu(Color.Goldenrod, Color.Gold, Texturen.AlgerianFont, screenWidth, screenHeight);
        ...
        public void StarteNeuesSpiel()
        {
            ...
            Sounds.Lademusik = new Soundsystem("Sounds\\138681__haydensayshi123__battle-march-action-loop.ogg", 0.15f, 1f, false);
        ...
        protected override void Draw(GameTime gameTime) // Ruft alle Draw Methoden auf
        {
            ...
            if (Hauptfenster.Tausch.SpielAktiv)
                ...
                {
                    if (Notizen.Kollision.collision(Help.GetMousePos(), Spiel2.players[Spiel2.CurrentPlayer].Notiz.pos[i] - Spiel2.Fenster, false))
                    {
                        overnotiz = true;
                        break;
                    }
            ...
            if (Hauptfenster.Tausch.SpielAktiv)
                if (!overnotiz)
                {
                    ...
                    for (int b = 0; b < Spiel2.players[t].pos.Count; b++)
                        if (Spiel2.players[t].PrüfeObKollision(b, new Vector2(mouseState.X + Spiel2.Fenster.X, mouseState.Y + Spiel2.Fenster.Y)))
                        {
                            overtank = true;
                            break;
                        }
            ...
            /*    spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, null, null, null, null, globalTransformation);
                ...
                spriteBatch.End();*/
            if (Haus.HAEUSER) mouseover = Haus.ZeichneHäuser(overnotiz || overtank ? true : false, spriteBatch, Game1.device, Spiel2);
            ...
            {
                spriteBatch.DrawString(Texturen.font2, "F9 Zug Beenden", new Vector2(screenWidth - 3 - 2 * ((float)Texturen.LeeresFeld.Width * scale5), ((float)Texturen.LeeresFeld.Height * scale5) + 42), Color.Lime);
            }
            if (Tastatur.TASTE_SPEICHERN.Wert) spriteBatch.DrawString(Texturen.font2, "F5 Speichern", new Vector2(screenWidth - 3 - 2 * ((float)Texturen.LeeresFeld.Width * scale5), ((float)Texturen.LeeresFeld.Height * scale5) + 54), Color.Lime);
            ...
            if (DEBUG_AKTIV.Wert)
            {
                ...
                for (int i = 0; i < Spiel2.Haeuser.Position.Count; i++)
                {
                    ...
                    spriteBatch.Draw(Texturen.dot, new Vector2(Spiel2.Haeuser.Position[i].X - (int)Spiel2.Fenster.X, Spiel2.Haeuser.Position[i].Y - (int)Spiel2.Fenster.Y - Texturen.haus[id].Height * Gebäudedaten.SKALIERUNG.Wert[id]), null, Color.Lime, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
                }

                for (int i = 0; i < Spiel2.Missile.Count(); i++)
                {
                    spriteBatch.Draw(Texturen.dot, new Vector2(Spiel2.Missile[i].RaktnSpitze().X - (int)Spiel2.Fenster.X, Spiel2.Missile[i].RaktnSpitze().Y - (int)Spiel2.Fenster.Y), null, Color.OrangeRed, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
                }
                ...
                for (int i = 0; i < Spiel2.players.Length && i < 1; i++)
                {
                    for (int c = 0; c < Spiel2.players[i].hp.Count && c < 1; c++)
                    {
                        ...
                        posx = posx + p.X;
                        posy = posy - p.Y;
                        spriteBatch.Draw(Texturen.dot, new Vector2(posx - (int)Spiel2.Fenster.X, posy - (int)Spiel2.Fenster.Y - Texturen.panzerindex[id].Height * Fahrzeugdaten.SCALEP.Wert[id]), null, Color.Lime, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
                        ...
                        for (int z = 0; z < Spiel2.Missile.Count(); z++)
                        {
                            ...
                            spriteBatch.Draw(Texturen.dot, new Vector2(RakPos.X - (int)Spiel2.Fenster.X, RakPos.Y - (int)Spiel2.Fenster.Y), null, Color.Yellow, 0.0f, new Vector2(0, 0), fact, SpriteEffects.None, 1);
                        }
                    }
                }
                ...
                for (int i = 0; i < Spiel2.players.Length; i++)
                {
                    for (int c = 0; c < Spiel2.players[i].hp.Count; c++)
                    {
                        int kind = Spiel2.players[i].KindofTank[c];
                        int welcher = Spiel2.players[i].overreach[c] ? 1 : 0;
                        Vector2 pos = Spiel2.players[i].pos[c] + new Vector2(welcher == 1 ? -Texturen.RohrPos[kind].X : Texturen.RohrPos[kind].X, -Texturen.RohrPos[kind].Y);//Texturen.CannonOrigin[kind][0] -
                        float temp2 = Texturen.CannonOrigin[kind][0].Y / 2;

                        Vector2 p = Help.RotatePositionOffset(pos, Spiel2.players[i].Angle[c], new Vector2(Texturen.panzerrohrindex[kind].Width - (Texturen.panzerrohrindex[kind].Width - Texturen.CannonOrigin[kind][0].X), welcher == 0 ? temp2 : -temp2));
                        p = Help.RotatePosition(Spiel2.players[i].pos[c], Spiel2.players[i].vehikleAngle[c], p);

                        spriteBatch.Draw(Texturen.dot, p - Spiel2.Fenster, null, Color.Yellow, 0, new Vector2(12.5f, 12.5f), fact, SpriteEffects.None, 1);
                        ...
                        for (int d = 0; d < Texturen.Radpositionen[Kindof].Count(); d++)
                        {
                            ...
                            Vector2 bew = Spiel2.players[i].logik[c].GetBewegungsVektor(d, Spiel2.players[i].vehikleAngle[c], Spiel2.players[i].pos[c], Spiel2.players[i].overreach[c]);
                            Vector2 add = bew * 10;
                            Vector2 p2 = Help.RotatePositionOffset(Spiel2.players[i].pos[c], vehikleAngle, new Vector2((overreach ? Texturen.Radpositionen[Kindof][d].X : -Texturen.Radpositionen[Kindof][d].X), Texturen.Radpositionen[Kindof][d].Y));
                            ...
                            if (i == Spiel2.CurrentPlayer && c == Spiel2.players[i].CurrentTank)
                                Help.DrawString(spriteBatch, Texturen.font2, bew.ToString(), new Vector2(10, 60 + d * 9), Color.DarkGoldenrod, Color.Black);
                ...
                // Zykluslinie
                int x = (int)(Spiel.Position(0));
                int y = Kartenformat.BottomOf(x, 0);
                Help.DrawLine(spriteBatch, new Vector2(Spiel.Position(0 - Spiel2.Fenster.X), 0), new Vector2(Spiel.Position(0 - Spiel2.Fenster.X), y), Color.Red, 2);

                // Mitte
                x = (int)(Spiel.Kartenbreite / 2);
                Help.DrawLine(spriteBatch, new Vector2(x - Spiel2.Fenster.X, 0), new Vector2(x - Spiel2.Fenster.X, Kartenformat.BottomOf(x, 0)), Color.Blue, 2);
            }

            #endregion DEBUG

            mouseover = Spiel2.players[Spiel2.CurrentPlayer].Notiz.Draw(spriteBatch, GraphicsDevice, Spiel2.Fenster, overnotiz ? false : true);
            ...
            Kurzmeldung.Zeichnen(spriteBatch, Texturen.font2, (int)Spiel2.Fenster.X, (int)(Spiel2.Fenster.X + screenWidth));

            int CurrentTank = Spiel2.players[Spiel2.CurrentPlayer].CurrentTank;

            if (Mod.SPIELERMENU_VISIBLE.Wert && CurrentTank >= 0) Spielermenu.Draw(spriteBatch, Texturen.font4, Spiel2.players[Spiel2.CurrentPlayer].Effekte[CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].Rucksack[CurrentTank], Spiel2, Transparenz);
            ...
            if (Spiel2.SpielerAktiv() < 2) { Hauptfenster.Tausch.SpielAktiv = false; DrawTextEnd(); }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (Spiel2.SpielerAktiv() > 1 && SpielAktiv)
            {
                ...
                if (!Client.isRunning)
                    if (Spiel2.Karte.collisions(Spiel2.Spielfeld, Spiel2.Missile, Spiel2.players, Spiel2.Haeuser, Spiel2.Baeume, Spiel2.Bunker, Spiel2.Kisten, Spiel2.Height, gameTime, new Vector2(Spiel2.Fenster.X + screenWidth / 2, Spiel2.Fenster.Y)))
                    {
                        Vordergrund.ErstelleVordergrund();
                    }

                Vordergrund.AktualisiereVordergrund(Spiel2.check_verzoegerte(gameTime));  // zündet die Raketen und zeichnet karte daraufhin neu
        ...
        private void createKasten()
        {
            ...
            Texturen.kasten = new Texture2D(this.GraphicsDevice, screenWidth, screenHeight);
            ...
            for (int x = 0; x < screenWidth; x++)
            {
                for (int y = 0; y < screenHeight; y++)
                {
                    ...
                    if ((x <= breite && y < screenHeight) || (x >= screenWidth - breite && y < screenHeight)) Spiel2.Colors[x + y * screenWidth] = Color.Gray;
                }
            }
            ...
            Texturen.dot = new Texture2D(this.GraphicsDevice, mas, mas);
            Texturen.dot2 = new Texture2D(this.GraphicsDevice, mas, mas);
        ...
        private void getBahn(Vector2 g, Vector2 schuss, Vector2 pos, int screenheight, int minx, int maxx, SpriteBatch spriteBatch)
        {
            Vector2 temp = pos;
            if (line == null)
            {
            ...
            {
                float angle = (float)Math.Atan2(schuss.Y, schuss.X);
                if (draw)
                {
        ...
        private void KeyboardKeys()
        {
            ...
            if (Spiel2.players[Spiel2.CurrentPlayer].Notiz.schreibend == true)
            {
                ...
                if (keybState != Keyboard.GetState() || last >= 15) { last = 0; Spiel2.players[Spiel2.CurrentPlayer].Notiz.TastenEingabe(Keyboard.GetState()); }
                keybState = Keyboard.GetState(); return;
            }

            if (Mod.EINGABEZEILE_VISIBLE.Wert && Editor.Textfelder != null && !Editor.Textfelder.visible) Eingabefenster.KeyboardKeys(keybState, Keyboard.GetState());
            ...
            if (Editor.visible && Editor.Textfelder != null && Editor.Textfelder.visible)
            {
                ...
                {
                    if ((currentState.IsConnected && currentState.Buttons.Start == Microsoft.Xna.Framework.Input.ButtonState.Pressed) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Escape)))
                    {
                        Editor.Textfelder.hide();

                        if (Editor.Textfeldtyp == 6)
                        {
                            Spiel2.players[Editor.TextfeldSpieler].Minen[Editor.TextfeldID] = Mine.Laden(Editor.Textfelder.Text, Spiel2.players[Editor.TextfeldSpieler].Minen[Editor.TextfeldID], Spiel2.players[Editor.TextfeldSpieler].Minen[Editor.TextfeldID].ID);
                        }
                        else
                            if (Editor.Textfeldtyp == 5)
                            {
                                Spiel2.Kisten.Laden(Editor.Textfelder.Text, Editor.TextfeldID, Content);
                            }
                            else
                                if (Editor.Textfeldtyp == 4)
                                {
                                    ...
                                    if (Editor.Textfeldtyp == 3)
                                    {
                                        ...
                                        if (Editor.Textfeldtyp == 2)
                                        {
                                            ...
                                            if (Editor.Textfeldtyp == 1)
                                            {
                                                Spiel2.players[Editor.TextfeldSpieler].TunnelAnlage[Editor.TextfeldID] = Tunnel.Laden(Editor.Textfelder.Text, Spiel2.players[Editor.TextfeldSpieler].TunnelAnlage[Editor.TextfeldID]);
                                            }
                                            else
                                                if (Editor.Textfeldtyp == 0)
                                                {
            ...
            if (!Editor.visible && Mod.ONLINE_ESCAPE_VISIBLE.Wert)
            {
                if (Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Escape))
                {
                    ...
                    SpielAktiv = false;
                    Hauptfenster.FormState.Restore(Hauptfenster.Program.Formular);

                    Hauptfenster.Tausch.SpielAktiv = false;

                    if (HTTP.HTTP.gameid != "")
                    {
                        TabPage[] qq = { Hauptfenster.Program.Formular.tabPage4, Hauptfenster.Program.Formular.tabPage5, Hauptfenster.Program.Formular.tabPage1 };
            ...
            if (Mod.EINGABEZEILE_VISIBLE.Wert && Eingabefenster.Eingabe.Sichtbar) { keybState = Keyboard.GetState(); return; }

            if (Spiel2 != null && keybState != Keyboard.GetState() && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.F2))
            {
                if (!Editor.visible)
                {
            ...
            if (Editor.Textfelder != null)
                if (Editor.Textfelder.visible)
                {
                    ...
                    if (keybState != Keyboard.GetState() || last >= 15) { last = 0; Editor.Textfelder.TastenEingabe(Keyboard.GetState()); }
                    keybState = Keyboard.GetState(); return;
                }

            if (SpielAktiv == false) { keybState = Keyboard.GetState(); return; }
            ...
            if ((currentState.IsConnected && currentState.Buttons.Y == Microsoft.Xna.Framework.Input.ButtonState.Pressed) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.I)))
            {
                ...
                for (int b = 0; b < Spiel2.players[t].pos.Count; b++)
                {
                    ...
                    if (Spiel2.players[t].PrüfeObKollision(b, Spiel2.players[t].pos[Spiel2.players[t].CurrentTank] + new Vector2(0, -5)))
                    {
                ...
                if (!found)
                {
                    for (int i = 0; i < Spiel2.Haeuser.Position.Count; i++)
                    {
                        ...
                        if (Spiel2.Haeuser.HausTyp[i] != Gebäudedaten.FABRIK && Spiel2.Haeuser.HausTyp[i] != Gebäudedaten.WAFFENHÄNDLER) continue;
                        if (Spiel2.Haeuser.IsCollision2(i, Spiel2.players[t].pos[Spiel2.players[t].CurrentTank] + new Vector2(0, -5)))
                        {
                            // auffüllen
            ...
            if (Spiel2 != null && (keybState != Keyboard.GetState() || oldcurrentState.Buttons.B != currentState.Buttons.B || currentState.Buttons.Back != oldcurrentState.Buttons.Back))
            {
                if (Tastatur.TASTE_FREIEBEWGUNG.Wert && (currentState.IsConnected && currentState.Buttons.B == Microsoft.Xna.Framework.Input.ButtonState.Pressed || Keyboard.GetState().IsKeyDown(Tastatur.FREIEBEWGUNG.Wert)))
                ...
                if (Mod.SPIELERMENU_VISIBLE.Wert && (keybState != Keyboard.GetState() || currentState.Buttons.Back != oldcurrentState.Buttons.Back))
                    if (Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.T))
                    {
                        ...
                        if ((!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Tab)) || (currentState.IsConnected && currentState.Buttons.Back == Microsoft.Xna.Framework.Input.ButtonState.Pressed))
            ...
            if (DEBUG_AKTIV.Wert)
            {
                ...
                if (Spiel2 != null && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.F10))
                {
            ...
            if (HTTP.HTTP.gameid != "" && Tastatur.TASTE_ZUG_BEENDEN.Wert && Spiel2 != null && Keyboard.GetState().IsKeyDown(Tastatur.ZUG_BEENDEN.Wert))
            {
                // Hochladen
                ...
                if (!Hauptfenster.Program.Formular.checkBox2.Checked)
                {
                    ...
                    else
                    {
                        ...
                        Hauptfenster.Program.Formular.FormBorderStyle = System.Windows.Forms.FormBorderStyle.FixedToolWindow;
                        Hauptfenster.Tausch.SpielAktiv = false;
                        if (HTTP.HTTP.gameid != "")
                        {
                            TabPage[] qq = { Hauptfenster.Program.Formular.tabPage4, Hauptfenster.Program.Formular.tabPage5, Hauptfenster.Program.Formular.tabPage1 };
            ...
            if (Tastatur.TASTE_SPEICHERN.Wert && Spiel2 != null && Keyboard.GetState().IsKeyDown(Tastatur.SPEICHERN.Wert))
            {
                // Nur Speichern
                String Pfad = "";
                if (HTTP.HTTP.gameid != "")
            ...
            if (Spiel2.CurrentPlayer >= 0 && Spiel2.CurrentPlayer < Spiel2.players.Count())
            {
                if (Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
                {
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        if (Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == i) continue;
            ...
            if ((Tastatur.TASTE_BEWEGUNG_LINKS.Wert || Tastatur.TASTE_BEWEGUNG_RECHTS.Wert) && Spiel2 != null && (!Tastatur.TASTE_BEWEGUNG_LINKS.Wert || (Tastatur.TASTE_BEWEGUNG_LINKS.Wert && Keyboard.GetState().IsKeyUp(Tastatur.BEWEGUNG_LINKS.Wert))) && (!Tastatur.TASTE_BEWEGUNG_RECHTS.Wert || (Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && Keyboard.GetState().IsKeyUp(Tastatur.BEWEGUNG_RECHTS.Wert))))
            {
                //keybState = Keyboard.GetState();
                if (Spiel2.players[Spiel2.CurrentPlayer].CurrentTank < 0) return;
                if (Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= Spiel2.players[Spiel2.CurrentPlayer].KindofTank.Count) return;
                // Sounds.PanzerMove2[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]].Stop();

                if (Sounds.PanzerMove[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]] != null)
                    Sounds.PanzerMove[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]].StopSound(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank);
            }
            ...
            if (Spiel2 != null && Spiel2.SpielerAktiv() > 1 && SpielAktiv)
            {
                ...
                if (Spiel2 != null && (currentState.IsConnected && currentState.Buttons.X == Microsoft.Xna.Framework.Input.ButtonState.Pressed || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.LeftShift))))
                {
                ...
                if (((currentState.IsConnected && currentState.ThumbSticks.Left.Y < 0.5f) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.BEWEGUNG_HOCH.Wert))) && ((currentState.IsConnected && currentState.ThumbSticks.Left.X > -0.5f) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.BEWEGUNG_RUNTER.Wert))))
                {
                ...
                if (Tastatur.TASTE_BEWEGUNG_HOCH.Wert && ((currentState.IsConnected && currentState.ThumbSticks.Left.Y > 0.5f) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_HOCH.Wert)))) // && !Keyboard.GetState().IsKeyDown(Tastatur.SCHUSS.Wert)
                {
                    if (Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2 && Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == 9)
                    {
                    }
                    else
                    {
                        ...
                        if (Spiel2.IsServer)
                        {
                            if (Spiel2.players[Spiel2.CurrentPlayer].overreach[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank])
                            {
                ...
                if (Tastatur.TASTE_BEWEGUNG_RUNTER.Wert && ((currentState.IsConnected && currentState.ThumbSticks.Left.Y < -0.5f) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RUNTER.Wert)))) // && !Keyboard.GetState().IsKeyDown(Tastatur.SCHUSS.Wert)
                {
                    if (Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2 && Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == 9)
                    {
                    }
                ...
                if (Spiel2.CurrentPlayer >= 0 && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
                    if ((!Spiel.TIMEOUT.Wert || Spiel2.Timeout > 0) && (!Spiel.SCHUESSE.Wert || Spiel2.Schuesse > 0) && (!Spieler.WAFFEN_COOLDOWN.Wert || Spiel2.players[Spiel2.CurrentPlayer].Cooldown[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] <= 0) && (!Spiel.MUNITION.Wert || Spiel2.players[Spiel2.CurrentPlayer].Munition[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank][Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] > 0) && (!Spiel.ACTION_POINTS.Wert || Spiel2.players[Spiel2.CurrentPlayer].ActionPoints >= Waffendaten.APKosten[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon]) && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 0)
                    {
                        if (Tastatur.TASTE_SCHUSS.Wert && ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Pressed) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Tastatur.SCHUSS.Wert))) &&
                            Fahrzeugdaten.Shootable[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] >= 1)
                        {
                            if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.bunker && Spiel2.PrüfeBunkerbau(Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]))
                            {
                                Spiel2.increaseshot = true;
                            }
                            else
                                if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.tunnel && Spiel2.PrüfeTunnelbau(Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]))
                                {
                                    Spiel2.increaseshot = true;
                                }
                                else
                                    if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.geschütz)
                                    {
                                        Spiel2.increaseshot = true;
                                    }
                                    else
                                        if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.geschütz2)
                                        {
                                            Spiel2.increaseshot = true;
                                        }
                        }

                        if (Tastatur.TASTE_SCHUSS.Wert && ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Released) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.SCHUSS.Wert))) && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 0)
                        {
                            if (Spiel2.increaseshot && (!Spiel.TIMEOUT.Wert || Spiel2.Timeout > 0))
                            {
                                ...
                                {
                                    if (Spiel2.PrüfeBunkerbau(Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]))
                                    {
                                        abgeschlossen = true;
                                        Spiel2.AddBunker(Spiel2.CurrentPlayer, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                    }
                                }
                                else
                                    if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.tunnel)
                                    {
                                        if (Spiel2.PrüfeTunnelbau(Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]))
                                        {
                                            abgeschlossen = true;
                                            Spiel2.AddTunnel(Spiel2.CurrentPlayer, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] - new Vector2(Texturen.tunnel.Width / 2 * Tunnel.SKALIERUNG, 0));
                                        }
                                    }
                                    else
                                        if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.geschütz)
                                        {
                                            abgeschlossen = true;
                                            Spiel2.AddGeschütz(Spiel2.CurrentPlayer, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                        }
                                        else
                                            if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.geschütz2)
                                            {
                                                abgeschlossen = true;
                                                Spiel2.AddGeschütz2(Spiel2.CurrentPlayer, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                            }

                                if (abgeschlossen)
                                {
                                    if (Spiel.MUNITION.Wert) Spiel2.players[Spiel2.CurrentPlayer].Munition[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank][Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon]--;
                                    if (Spieler.WAFFEN_COOLDOWN.Wert) Spiel2.players[Spiel2.CurrentPlayer].Cooldown[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = Waffendaten.APKosten[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon];
                                    if (Spiel.ACTION_POINTS.Wert) Spiel2.players[Spiel2.CurrentPlayer].ActionPoints -= Waffendaten.APKosten[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon];
                        ...
                        if (Spiel2.CurrentPlayer >= 0 && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
                            if ((!Spiel.TIMEOUT.Wert || Spiel2.Timeout > 0) && (!Spiel.SCHUESSE.Wert || Spiel2.Schuesse > 0) && (!Spieler.WAFFEN_COOLDOWN.Wert || Spiel2.players[Spiel2.CurrentPlayer].Cooldown[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] <= 0) && (!Spiel.MUNITION.Wert || Spiel2.players[Spiel2.CurrentPlayer].Munition[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank][Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] > 0 || Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 5) && (!Spiel.ACTION_POINTS.Wert || Spiel2.players[Spiel2.CurrentPlayer].ActionPoints >= Waffendaten.APKosten[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon]) && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] > 0)
                            {
                                if (Tastatur.TASTE_SCHUSS.Wert && ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Pressed) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Tastatur.SCHUSS.Wert))) &&
                                    Fahrzeugdaten.Shootable[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] >= 1)
                                {
                                    //   keybState = Keyboard.GetState();
                                    if (!Spiel2.increaseshot && !Spiel2.increaseairstrike) Spiel2.players[Spiel2.CurrentPlayer].shootingPower = Waffendaten.minShootingpower[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon];
                                ...
                                if (Tastatur.TASTE_SCHUSS.Wert && ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Released) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.SCHUSS.Wert)) || ((Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 4 || Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 5) && Spiel2.increaseshot)) && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] > 0)
                                {
                                    bool ok = true;
                                    ...
                                    if (Spiel2.increaseshot && (!Spiel.TIMEOUT.Wert || Spiel2.Timeout > 0) && ok)
                                    {
                                        ...
                                        Vector2 a = Spiel2.players[Spiel2.CurrentPlayer].Rohrspitze(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank);

                                        if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 5) if (Spiel.MUNITION.Wert) Spiel2.players[Spiel2.CurrentPlayer].Munition[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank][Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon]--;
                                        if (Spieler.WAFFEN_COOLDOWN.Wert) Spiel2.players[Spiel2.CurrentPlayer].Cooldown[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = Waffendaten.APKosten[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon];
                                        if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 5) if (Spiel.ACTION_POINTS.Wert) Spiel2.players[Spiel2.CurrentPlayer].ActionPoints -= Waffendaten.APKosten[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon];
                                        if (Spiel.SCHUESSE.Wert) Spiel2.Schuesse--;

                                        if (!Spiel2.increaseairstrike)
                                        {
                                            if (Fahrzeugdaten.Shootable[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] >= 1 && (Spiel2.players[Spiel2.CurrentPlayer].shootingPower > 2 || Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 3))
                                            {
                                                /*if (Client.isRunning)
                                                {
                                                ...
                                                if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 3)
                                                {
                                                    // Objektezugriffe, werden nicht verschossen, aber durch schuss ausgelöst
                                                    Spieler tmp = Spiel2.players[Spiel2.CurrentPlayer];
                                                    int id = tmp.CurrentTank;
                                                    Spiel2.players[Spiel2.CurrentPlayer].Minen.Add(new Mine((int)tmp.pos[id].X, (int)tmp.pos[id].Y, (int)(Waffendaten.Daten2[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon].Z), Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon, Spiel2.players[Spiel2.CurrentPlayer].Minen.Count));
                                                }
                                                else
                                                {
                                                    // normale Rakete
                                                    ...
                                                    float angle = Spiel2.players[Spiel2.CurrentPlayer].Angle[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank];
                                                    Vector2 up = new Vector2(0, -1);
                                                    if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 4)
                                                    {
                                                        float merk = Spiel2.players[Spiel2.CurrentPlayer].shootingPower;
                                                        Spiel2.players[Spiel2.CurrentPlayer].shootingPower = 10 + Spiel.incshoot;//Spiel.rand.Next(50, (int) (Spiel2.players[Spiel2.CurrentPlayer].shootingPower+1));
                                                        Matrix rotMatrix = Matrix.CreateRotationZ(Spiel2.players[Spiel2.CurrentPlayer].Angle[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] + Spiel2.players[Spiel2.CurrentPlayer].vehikleAngle[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] - (float)MathHelper.PiOver2 + MathHelper.ToRadians(Spiel.rand.Next(-8, 9)));
                                                        ...
                                                        Spiel2.CurrentMissile = Spiel2.AddRakete(Spiel2.CurrentPlayer, a, b, 10, Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon, Spiel2.players[Spiel2.CurrentPlayer].CurrentTank);
                                                        Spiel2.players[Spiel2.CurrentPlayer].shootingPower = merk;
                                                    }
                                                    else
                                                        if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 5)
                                                        {
                                                            // reparieren, erobern
                                                            if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.reparieren)
                                                            {
                                                                // Haus Reparieren
                                                                ...
                                                                for (int i = 0; i < Spiel2.Haeuser.Position.Count; i++)
                                                                {
                                                                    if (Spiel2.Haeuser.Lebenspunkte[i] >= Spiel2.Haeuser.MaximaleLebenspunkte[i]) continue;
                                                                    if (Spiel2.Haeuser.IsCollision2(i, Spiel2.players[player].pos[t] + new Vector2(0, -2)))
                                                                    {
                                                                        int rep = Help.RepariereBild(Spiel2.Haeuser.Bild[i], Texturen.haus[Spiel2.Haeuser.HausTyp[i]], 25);
                                                                ...
                                                                if (Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == Waffendaten.erobern)
                                                                {
                                                                    // Haus erobern
                                                                    int player = Spiel2.CurrentPlayer;
                                                                    int t = Spiel2.players[player].CurrentTank;
                                                    ...
                                                    {
                                                        // MG
                                                        Spiel2.Karte.AddExplosion(Spiel2.Karte.particleListExp, a, 4, 5, 150, Time, new Vector3(0.7f, 1f, 1.2f), 18, 0);
                                                    }
                                                    else
                                                        if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 5)
                                                        {
                                                            ...
                                                            Spiel2.Karte.AddExplosion(Spiel2.Karte.particleListExp, a, 10, 35, 650, Time, new Vector3(0.7f, 1f, 1.2f), 18, 0);

                                                    if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 4 && Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] != 5) Spiel2.Missile[Spiel2.CurrentMissile].focused = true; //CurrentMissle
                                                    if ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Released) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.SCHUSS.Wert))) Spiel2.players[Spiel2.CurrentPlayer].shootingPower = 0;
                                                }
                                            }

                                            if ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Released) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.SCHUSS.Wert)))
                                            {
                                                Spiel2.increaseshot = false;
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            //     if (Rakete.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 1)
                                            {
                                                // Abstand
                                                a.X = Spiel2.players[Spiel2.CurrentPlayer].shootingPower;
                                                Spiel2.CurrentMissile = Spiel2.AddRakete(Spiel2.CurrentPlayer, new Vector2(a.X, Spiel.rand.Next(-1100, -200)), new Vector2(Spiel.rand.Next(-100, 100) / 25, -1), 300 * 4, Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon, Spiel2.players[Spiel2.CurrentPlayer].CurrentTank);

                                                if (Spiel2.CurrentMissile != -1) Spiel2.Missile[Spiel2.CurrentMissile].focused = true; //CurrentMissle
                                ...
                                if (!Spiel2.increaseshot && ((currentState.IsConnected && currentState.Buttons.A == Microsoft.Xna.Framework.Input.ButtonState.Released) || (!currentState.IsConnected && Keyboard.GetState().IsKeyUp(Tastatur.SCHUSS.Wert))))
                                {
                                    Spiel2.increaseairstrike = false;
                                    Spiel2.increaseshot = false;
                                    Spiel2.players[Spiel2.CurrentPlayer].shootingPower = 0;
            ...
            if (keybState != Keyboard.GetState() || currentState.Buttons != oldcurrentState.Buttons || (currentState.Triggers.Left >= 0.1f && oldcurrentState.Triggers.Left < 0.1f) || (currentState.Triggers.Right >= 0.1f && oldcurrentState.Triggers.Right < 0.1f))
            {
                #region Panzerwahl

                Spieler current = Spiel2.players[Spiel2.CurrentPlayer];
                if (Tastatur.TASTE_FAHRZEUGWAHL_LINKS.Wert && (currentState.IsConnected && currentState.Buttons.LeftShoulder == Microsoft.Xna.Framework.Input.ButtonState.Pressed || Keyboard.GetState().IsKeyDown(Tastatur.FAHRZEUGWAHL_LINKS.Wert)) && current.getPanzerID(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank, current.OrdneEigenePanzerAnhandDerKarte()) > 0)
                {
                    keybState = Keyboard.GetState();
                    List<int> list = current.OrdneEigenePanzerAnhandDerKarte();
                    Spiel2.players[Spiel2.CurrentPlayer].CurrentTank = list[current.getPanzerID(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank, list) - 1];
                    int a = Spiel2.players[Spiel2.CurrentPlayer].CurrentTank;
                    if (Spiel2.increaseairstrike)
                    {
                        Spiel2.players[Spiel2.CurrentPlayer].shootingPower = Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].X;
                        if (!Spiel2.Moving_Map) Spiel2.Set_Focus(new Vector2(Spiel2.players[Spiel2.CurrentPlayer].shootingPower, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].Y));
                    }
                    else
                        if (!Spiel2.Moving_Map) Spiel2.Set_Focus(new Vector2(Spiel2.players[Spiel2.CurrentPlayer].shootingPower, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].Y));
                ...
                if (Tastatur.TASTE_FAHRZEUGWAHL_RECHTS.Wert && (currentState.IsConnected && currentState.Buttons.RightShoulder == Microsoft.Xna.Framework.Input.ButtonState.Pressed || Keyboard.GetState().IsKeyDown(Tastatur.FAHRZEUGWAHL_RECHTS.Wert)) && current.getPanzerID(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank, current.OrdneEigenePanzerAnhandDerKarte()) < Spiel2.players[Spiel2.CurrentPlayer].pos.Count - 1)
                {
                    keybState = Keyboard.GetState();
                    List<int> list = current.OrdneEigenePanzerAnhandDerKarte();
                    Spiel2.players[Spiel2.CurrentPlayer].CurrentTank = list[current.getPanzerID(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank, list) + 1];
                ...
                if (Tastatur.TASTE_WAFFENWAHL_LINKS.Wert && ((currentState.IsConnected && currentState.Triggers.Left >= 0.1f) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Tastatur.WAFFENWAHL_LINKS.Wert))))
                {
                    keybState = Keyboard.GetState();
                    ...
                    for (; Fahrzeugdaten.Shootable[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[CurrentTankNow], CurrentWeaponNow] <= 0; )
                    {
                        CurrentWeaponNow--;
                        if (CurrentWeaponNow < 0) CurrentWeaponNow = Waffendaten.Daten.Count() - 1;
                    ...
                    {
                        Spiel2.increaseairstrike = true;
                        Spiel2.players[Spiel2.CurrentPlayer].shootingPower = Spiel2.players[Spiel2.CurrentPlayer].pos[CurrentTankNow].X;
                        if (!Spiel2.Moving_Map) Spiel2.Set_Focus(new Vector2(Spiel2.players[Spiel2.CurrentPlayer].shootingPower, Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].Y));
                    }
                    else
                    {
                ...
                if (Tastatur.TASTE_WAFFENWAHL_RECHTS.Wert && ((currentState.IsConnected && currentState.Triggers.Right >= 0.1f) || (!currentState.IsConnected && Keyboard.GetState().IsKeyDown(Tastatur.WAFFENWAHL_RECHTS.Wert))))
                {
                    keybState = Keyboard.GetState();
            ...
            if (Tastatur.TASTE_LINKS.Wert && ((currentState.IsConnected && currentState.ThumbSticks.Right.X < 0.0f) || Keyboard.GetState().IsKeyDown(Tastatur.LINKS.Wert)))
            {
                if (Mod.FREIE_KARTENBEWEGUNG.Wert && Spiel2.Moving_Map)
                {
                    ...
                    if (Waffendaten.Verschiessbar[Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon] == 2)
                    {
                        ...
                        Spiel2.players[Spiel2.CurrentPlayer].shootingPower = Spiel.Position(Spiel2.players[Spiel2.CurrentPlayer].shootingPower);
            ...
            if (Tastatur.TASTE_RECHTS.Wert && ((currentState.IsConnected && currentState.ThumbSticks.Right.X > 0.0f) || Keyboard.GetState().IsKeyDown(Tastatur.RECHTS.Wert)))
            {
                if (Mod.FREIE_KARTENBEWEGUNG.Wert && Spiel2.Moving_Map)
                {
                    Spiel2.Set_Focus_X(new Vector2(Spiel2.Next_Fenster.X + 25 + screenWidth / 2, Spiel2.Next_Fenster.Y));
            ...
            {
                if (Tastatur.TASTE_BEWEGUNG_RUNTER.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RUNTER.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                {
                    Kartenfunktionen.Graben_Runter(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                    wait = 20;
                }

                if (Tastatur.TASTE_BEWEGUNG_LINKS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_LINKS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                {
                    Kartenfunktionen.Graben_Links(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                    wait = 20;
                }

                if (Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RECHTS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                {
                    Kartenfunktionen.Graben_Rechts(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                    wait = 20;
                }
            }
            else
                if (wait <= 0 && Spiel2.players[Spiel2.CurrentPlayer].CurrentWeapon == 9)
                {
                    if (Tastatur.TASTE_BEWEGUNG_RUNTER.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RUNTER.Wert) && Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RECHTS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                    {
                        Spiel2.players[Spiel2.CurrentPlayer].overreach[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = true;
                        Kartenfunktionen.Bauen_Rechts_Runter(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                        wait = 20;
                    }
                    else
                        if (Tastatur.TASTE_BEWEGUNG_RUNTER.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RUNTER.Wert) && Tastatur.TASTE_BEWEGUNG_LINKS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_LINKS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                        {
                            Spiel2.players[Spiel2.CurrentPlayer].overreach[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = false;
                            Kartenfunktionen.Bauen_Links_Runter(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                            wait = 20;
                        }
                        else
                            if (Tastatur.TASTE_BEWEGUNG_HOCH.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_HOCH.Wert) && Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RECHTS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                            {
                                Spiel2.players[Spiel2.CurrentPlayer].overreach[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = true;
                                Kartenfunktionen.Bauen_Rechts_Hoch(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                wait = 20;
                            }
                            else
                                if (Tastatur.TASTE_BEWEGUNG_HOCH.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_HOCH.Wert) && Tastatur.TASTE_BEWEGUNG_LINKS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_LINKS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                                {
                                    Spiel2.players[Spiel2.CurrentPlayer].overreach[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = false;
                                    Kartenfunktionen.Bauen_Links_Hoch(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                    wait = 20;
                                }
                                else
                                    if (Tastatur.TASTE_BEWEGUNG_HOCH.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_HOCH.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                                    {
                                        Spiel2.players[Spiel2.CurrentPlayer].overreach[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = false;
                                        Kartenfunktionen.Bauen_Hoch(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                        wait = 20;
                                    }
                                    else
                                        if (Tastatur.TASTE_BEWEGUNG_LINKS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_LINKS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                                        {
                                            Kartenfunktionen.Bauen_Links(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                            wait = 20;
                                        }
                                        else
                                            if (Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RECHTS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] == 2)
                                            {
                                                Kartenfunktionen.Bauen_Rechts(Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]);
                                                wait = 20;
                                            }
                }
            // }

            if (Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && Keyboard.GetState().IsKeyUp(Tastatur.BEWEGUNG_RECHTS.Wert) && Tastatur.TASTE_BEWEGUNG_LINKS.Wert && Keyboard.GetState().IsKeyUp(Tastatur.BEWEGUNG_LINKS.Wert) && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
            {
                Spiel2.players[Spiel2.CurrentPlayer].logik[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].Motoren.Abschalten();
            }

            if (Tastatur.TASTE_BEWEGUNG_LINKS.Wert && ((currentState.IsConnected && currentState.ThumbSticks.Left.X < -0.5f) || Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_LINKS.Wert)) && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
            {
                // keybState = Keyboard.GetState();
                if (!Spiel2.players[Spiel2.CurrentPlayer].ImTunnel || (keybState != Keyboard.GetState() || currentState.ThumbSticks.Left.X < -0.5f))
                    if (Spiel2.IsServer)
                    {
                        Spiel2.Current_Left();
                    }
                ...
                Spiel2.players[Spiel2.CurrentPlayer].logik[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].Motoren.Beschleunigen();

                int dist = 0;
                if (Spiel2.Fenster.X > Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].X)
                {
                    dist = (int)(Spiel2.Fenster.X - Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].X);
                }
                else
                    dist = (int)(Spiel2.players[Spiel2.CurrentPlayer].pos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank].X - (Spiel2.Fenster.X));

                float wert = Fahrzeugdaten.VOLUMES.Wert[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]];
                float a = (float)((float)wert - ((float)wert / 2048) * dist);
                if (a < 0) a = 0;

                if (Sounds.PanzerMove[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]] != null)
                {
                    Sounds.PanzerMove[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]].ResumeSound(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank);
                    Sounds.PanzerMove[Spiel2.players[Spiel2.CurrentPlayer].KindofTank[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank]].SetVolume(Spiel2.players[Spiel2.CurrentPlayer].CurrentTank, a);
            ...
            if (Tastatur.TASTE_BEWEGUNG_RECHTS.Wert && ((currentState.IsConnected && currentState.ThumbSticks.Left.X > 0.5f) || Keyboard.GetState().IsKeyDown(Tastatur.BEWEGUNG_RECHTS.Wert)) && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
            {
                //keybState = Keyboard.GetState();
                if (!Spiel2.players[Spiel2.CurrentPlayer].ImTunnel || (keybState != Keyboard.GetState() || currentState.ThumbSticks.Left.X > 0.5f))
                    if (Spiel2.IsServer)
        ...
        private void MouseKeys()
        {
            ...
            if (!Editor.visible && Spiel2 != null && Hauptfenster.Tausch.SpielAktiv)
            {
                if (!Spielermenu.visible)
                {
                    ...
                    if (!BauMenü.visible)
                    {
                        ...
                        {
                            Spiel2.players[Spiel2.CurrentPlayer].Notiz.Textfelder[Spiel2.players[Spiel2.CurrentPlayer].Notiz.selected].MouseKeys(Spiel2.Fenster);
                        }

                        if (mouseState != Help.GetMouseState())
                            notizbereichleftklicked = Spiel2.players[Spiel2.CurrentPlayer].Notiz.Notizbereich_klick(Spiel2.Fenster, GraphicsDevice);
                        ...
                        if (!Spiel2.players[Spiel2.CurrentPlayer].Notiz.schreibend)
                        {
                            notizrightklicked = Spiel2.players[Spiel2.CurrentPlayer].Notiz.MouseKeys(Spiel2.Fenster, GraphicsDevice, mouseState);

                            if (Clickbool == false && Click < 2 && Help.GetMouseState().LeftButton == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
                            {
                                Clickbool = true;
                            }
                            if (Click == 1)
                            ...
                            if (Clickbool == true && Help.GetMouseState().LeftButton == Microsoft.Xna.Framework.Input.ButtonState.Released)
                            {
                                Click = Click + 1;
                                Clickbool = false;
                            }
            ...
            if (!Editor.visible && Mod.SPIELERMENU_VISIBLE.Wert)
                if (CurrentTank >= 0)
                {
                    if (Spielermenu.mouseKeys(mouseState, Spiel2.players[Spiel2.CurrentPlayer].Rucksack[CurrentTank], Spiel2.players[Spiel2.CurrentPlayer].Effekte[CurrentTank]))
                    {
            ...
            if (pregamemenuaufruf)
            {
                SetUpMenu.MouseKeys(mouseState, spriteBatch);
                        ...
                        case 2:
                            {
                                ...
                                else pauseMenu.show(); break;
                            }
                        default: { break; }
                    }
                    #endregion

                    if (!Hauptfenster.Tausch.SpielAktiv) { mouseState = Help.GetMouseState(); return; }
                    if (mouseState.LeftButton == Help.GetMouseState().LeftButton && mouseState.RightButton == Help.GetMouseState().RightButton) { mouseState = Help.GetMouseState(); return; }
                    ...
                    if (Help.GetMouseState().LeftButton == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
                    {
                        ...
                        {
                            if (Spiel2.players[c].PrüfeObKollision(i, new Vector2(mouseState.X + Spiel2.Fenster.X, mouseState.Y + Spiel2.Fenster.Y)))
                            {
                        ...
                        if (!found)
                            for (int i = 0; i < Spiel2.Haeuser.Position.Count; i++)
                            {
                                ...
                                if (Spiel2.Haeuser.IsCollision2(i, new Vector2(mouseState.X + Spiel2.Fenster.X, mouseState.Y + Spiel2.Fenster.Y)))
                                {
                                    // Fabrik gewählt
            ...
            if (!Editor.visible && !notizrightklicked && Help.GetMouseState().RightButton == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
            {
                if (Spiel2.CurrentPlayer >= 0 && Spiel2.players[Spiel2.CurrentPlayer].CurrentTank >= 0)
                {
                    // Lasse den Panzer fahren
                    Spiel2.players[Spiel2.CurrentPlayer].Zielpos[Spiel2.players[Spiel2.CurrentPlayer].CurrentTank] = new Vector2(mouseState.X + Spiel2.Next_Fenster.X, Kartenformat.BottomOf(mouseState.X + Spiel2.Next_Fenster.X, mouseState.Y + Spiel2.Next_Fenster.Y));
                }
            }
