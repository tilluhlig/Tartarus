...
namespace _4_1_
{
    ...
    public class Spiel
    {
        ...
        public static int incshoot
        {
            get
            {
                incshoot2++; incshoot2 %= 5;
                incshoot3++; incshoot3 %= 9;
        ...
        {
            int breite = (int)(Texturen.bunker[0].Width * Optimierung.Skalierung(0.25f));
            int id = Spieler;
        ...
        public void AddPanzer(int Spieler, int typ, float _angle, bool _overreach, Vector2 _pos)
        {
            ...
            if (names == null || names.Count == 0)
            {
                if (File.Exists("Content\\Konfiguration\\Namen.txt"))
                {
                    ...
                    ReaderStream.ReaderStream datei = new ReaderStream.ReaderStream("Content\\Konfiguration\\Namen.txt");
            ...
            players[Spieler].Size.Add((float)Fahrzeugdaten.SCALEP.Wert[di]);
            ...
            {
                players[Spieler].logik[players[Spieler].logik.Count - 1].Motoren.AddRad(Texturen.Radpositionen[di][d], Texturen.panzerindexreifen[di]);
            }
        ...
        /// <returns>System.Int32.</returns>
        public int AddRakete(int Spieler, Vector2 pos, Vector2 Direction, int Lebensdauer, int Weapon, int CurrentTank) // Fügt eine Rakete zum Spiel hinzu  // geändert
        {
            ...
            for (int i = 0; i < Missile.Length; i++)
            {
                ...
                if (Missile[i].Besitzer[0] == -1)
                {
                    ...
                    if (Spieler <= 2)
                    {
                        dist = (int)(players[Spieler].pos[players[Spieler].CurrentTank].X - (Fenster.X)); if (dist < 0) dist = -dist;
                    }
                    else
                        dist = 0; // ?????

                    //sh[Waffendaten.Abschuesse[Weapon]].Play((float)(1.0d - ((double)dist / Spielfeld.Length)), 0.0f, 0.0f);
                    sh[Waffendaten.Abschuesse[Weapon]].PlaySoundAny(false, (float)(1.0d - ((double)dist / Spielfeld.Length)));
        ...
        public void Airstrike(Vector2 pos, int Spieler)// löst einen Airstrike aus
        {
            ...
            int[] q = new int[9]; // anzahl der raketen, ungerade anzahl benötigt
            for (int i = -(q.Length / 2); i <= (q.Length / 2); i++)
            {                                                              // Abstand
                q[i + (q.Length / 2)] = AddRakete(Spieler, new Vector2(pos.X + (i * 80), rand.Next(-1100, -200)), new Vector2(rand.Next(-100, 100) / 25, -1), 300 * 4, 5, players[CurrentPlayer].CurrentTank);
            }
        ...
        public void check_Focus() // prüft ob zielposition des Fensters erreicht, bewegt fenster notfalls
        {
            ...
            {
                int add = (int)(Next_Fenster.X - Fenster.X);
                if (add < 0) add = -add;
                ...
                if (!Moving_Map)
                {
                    if (add < 100)
                    {
                        ...
                        if (add < 500)
                        {
                            if (add > 14) { add = 25; } // 14,14
                        }
                        else
                            if (add < 1000)
                            {
                                if (add > 20) { add = 35; } // 20,21
                            }
                            else
                                if (add > 25) { add = 50; } // 35,35
                }
        ...
        /// </summary>
        /// <param name="gameTime">The game time.</param>
        public void check_players(GameTime gameTime) // prüfe timeout der Spieler und ob spieler tot/aus spielfeld gefallen
        {
            //  if (Client.isRunning) return;
            ...
            for (int i = 0; i < players.Length; i++)
            {
                for (int b = 0; b < players[i].pos.Count; b++)
                {
                    ...
                    if (players[i].Effekte[b].GetHP(players[i].hp[b]) <= 0)
                    {
                        ...
                        if (players[i].pos[b].Y >= Height) { Sounds.PanzerMove[Fahrzeugdaten.SINKSOUND.Wert[players[i].KindofTank[b]]].StopSound(b); Sounds.sink[Fahrzeugdaten.SINKSOUND.Wert[players[i].KindofTank[b]]].PlaySoundAny(); players[i].isthere[b] = false; if (Server.isRunning) Server.Send("ISTHERE " + i + " " + b + " false"); }
                    ...
                    if (players[i].CurrentLv[b] < 3)
                    {
                        ...
                        players[i].ExpNow[b] = players[i].ExpNow[b] * 100 / Fahrzeugdaten.ExpToLvUp[players[i].KindofTank[b], players[i].CurrentLv[b]];
                        if (players[i].ExpNow[b] >= Fahrzeugdaten.ExpToLvUp[players[i].KindofTank[b], players[i].CurrentLv[b]])
                        {
                            players[i].ExpNow[b] -= Fahrzeugdaten.ExpToLvUp[players[i].KindofTank[b], players[i].CurrentLv[b]];
                ...
                for (int b = 0; b < players[i].pos.Count; b++)
                {
                 ...
                 //   geschv = 1.0f;
                    ...
                    players[i].logik[b].Motoren.Raeder[0].Energie -= new Vector2(0, y);
                  {
                    ...
                    int ditt = (int)(Texturen.panzerindex[players[i].KindofTank[b]].Width * (players[i].Size[b]));
                    ditt = Fahrzeugdaten.FAHRM.Wert[players[i].KindofTank[b]];
                    int add = ditt / 2;
                    if ((int)players[i].pos[b].X - ditt + add < 0) ditt = 0;
                    if ((int)players[i].pos[b].X - ditt + add > Spielfeld.Length - 2) ditt = (Spielfeld.Length) - ((int)players[i].pos[b].X - 2 - ditt + add);

                    int playerposX = (int)players[i].pos[b].X + add;
                    int playerposY = (int)players[i].pos[b].Y;
                    int playerposX2;
                    int playerposY2;
                    float summ = 0;
                    int dit = 0;
                    ...
                    {
                        summ += Kartenformat.BottomOf(playerposX - c + add, playerposY);// Spielfeld[playerposX - c + add];
                        dit++;
                    }
                    summ /= dit;
                    ...
                    float auxangle = (float)Math.Atan((double)(Kartenformat.BottomOf(playerposX2, playerposY2) - Kartenformat.BottomOf(playerposX, playerposY)) / ((playerposX2) - (playerposX))); // (float)Math.Atan((double)(Spielfeld[playerposX2] - Spielfeld[playerposX]) / ((playerposX2) - (playerposX)));
        ...
        public void check_Raketen() // muss jeden takt aufgerufen werden, verkürzt lebensdauer der raketen
        {
            ...
            for (int i = 0; i < Missile.Length; i++)
            {
                ...
                if (Missile[i].focused && CurrentMissile != -1)
                {
                    ...
                    if (!Moving_Map) Set_Focus(Spiel.Position(Missile[i].misslePosition)); focused_rakete = true;
                }
            }
            if (wait_change > 0) wait_change--;
            if (focused_rakete && !Moving_Map) { Set_Focus(Spiel.Position(Missile[CurrentMissile].misslePosition)); wait_change = (int)Waffendaten.Daten2[Missile[CurrentMissile].Art].X; }
            if (!focused_rakete && wait_change == 0 && increaseairstrike == false && !Moving_Map && players[CurrentPlayer].CurrentTank > -1) Set_Focus_X(Spiel.Position(players[CurrentPlayer].pos[players[CurrentPlayer].CurrentTank]));
        }
        ...
        public void check_shot_increase() // aktualisiert die sshootingpower, wenn leertaste gedrückt
        {
            if (increaseshot == true)
            {
                ...
                {
                    //  if (!Moving_Map) Set_Focus(new Vector2(players[CurrentPlayer].shootingPower, players[CurrentPlayer].pos[players[CurrentPlayer].CurrentTank].Y));
                }
            }
        }
        ...
        public List<Vector3> check_verzoegerte(GameTime gameTime) // prüft ob nach dem Aufschlag verzögerte Raketen jetzt zünden dürfen
        {
            ...
            for (int i = 0; i < Missile.Length; i++)
            {
                if (Missile[i].verzoegerung > 0)
                {
                    ...
                    if (Missile[i].verzoegerung == 0)
                    {
                        ...
                        {
                            list.AddRange(Explosionsschäden(gameTime, Missile[i].RaktnSpitze(), Missile[i].Energie, Missile[i].Art, Missile[i].Besitzer));
                        }
        ...
        public void Current_Left() // bewegt den aktuellen Panzer nach links
        {
            ...
            if (Spieler.PLAYER_FUEL.Wert && players[CurrentPlayer].Rucksack[players[CurrentPlayer].CurrentTank].GibTreibstoff() < Fahrzeugdaten.VERBRAUCH.Wert[players[CurrentPlayer].KindofTank[players[CurrentPlayer].CurrentTank]]) return;
            if (Spiel.TIMEOUT.Wert && Timeout <= 0) return;

            bool message = players[CurrentPlayer].Links(Spielfeld, players[CurrentPlayer].CurrentTank);
            if (message)
            {
                if (Spiel.TIMEOUT.Wert) Timeout -= 5;
        ...
        public bool Current_Rohr_Left() // bewegt den aktuellen Panzer nach rechts
        {
            ...
            if (Server.isRunning) Server.Send("ROHRANGLE " + CurrentPlayer + " " + players[CurrentPlayer].CurrentTank + " " + players[CurrentPlayer].Angle[players[CurrentPlayer].CurrentTank]);
            if (Server.isRunning) Server.Send("OVERREACH " + CurrentPlayer + " " + players[CurrentPlayer].CurrentTank + " " + players[CurrentPlayer].overreach[players[CurrentPlayer].CurrentTank]);
            ...
            if (result)
            {
                if (Sounds.Panzer_rohr_end[typ] != null)
                {
                    ...
                    if (Sounds.Panzer_rohrmode[typ] == 0 && !Sounds.Panzer_rohr_begin[typ].IsPlaying(soundid))
                    {
                        ...
                        if (Sounds.Panzer_rohrmode[typ] == 1 && !Sounds.Panzer_rohr_loop[typ].IsPlaying(soundid) && !Sounds.Panzer_rohr_begin[typ].IsPlaying(soundid))
                        {
        ...
        public List<Vector3> Explosionsschäden(GameTime gameTime, Vector2 pos, int Energie, int Art, int[] Besitzer)
        {
            ...
            for (int b = 0; b < players.Length; b++)
            {
                for (int c = 0; c < players[b].pos.Count; c++)
                {
                    ...
                    int dist = (int)Math.Sqrt(Math.Pow(players[b].pos[c].X - pos.X, 2) + Math.Pow(players[b].pos[c].Y - pos.Y, 2));
                    if (dist < 0) dist = -dist;
                    int energy = (int)((float)Energie * Waffendaten.Energiefaktor[Art]);
                    ...
                    { // rakete ist eine freeze
                        ...
                        if ((Art == 3 || Art == 12) && !players[b].Effekte[c].GetVergiftet())
                        {
                            // Giftgas
                            ...
                            if ((Art == 13) && !players[b].Effekte[c].GetElektrisiert())
                            {
                                // Elektrisiert
                    ...
                    if (found > 0)
                    {
                        ...
                        {
                            Vector2 dat = players[b].pos[c] + new Vector2((float)-Texturen.panzerindex[typ].Width / 2 * Fahrzeugdaten.SCALEP.Wert[typ], (float)-Texturen.panzerindex[typ].Height * Fahrzeugdaten.SCALEP.Wert[typ]) + Fahrzeugdaten.Messpunkte[typ][d];
                        ...
                        int schaden = (int)(((float)((float)found / gesamt) * Waffendaten.Zentrumschaden[Art]) * 0.25f + (float)((float)1 - ((float)abstand / energy)) * Waffendaten.Zentrumschaden[Art] * 0.75f);
                        if (schaden > 0 && Waffendaten.Verschiessbar[Art] != 4) Kurzmeldung.Hinzufügen("-" + schaden.ToString(), new Vector2(players[b].pos[c].X, players[b].pos[c].Y - Texturen.panzerindex[players[b].KindofTank[c]].Height * Fahrzeugdaten.SCALEP.Wert[players[b].KindofTank[c]]), Color.Yellow);//(players[b].Farbe)
                    ...
                    {
                        // panzer entfernen
                        Nutzloses.Hinzufügen(Texturen.panzerruine[players[b].KindofTank[c]], players[b].pos[c], players[b].vehikleAngle[c], players[b].overreach[c], players[b].Size[c], true, true);

                        if (Besitzer[0] >= 0 && Besitzer[1] >= 0)
                        {
                            int punkte = (int)(Fahrzeugdaten.ExpRewarded[players[b].KindofTank[c]] * ((players[b].CurrentLv[c]) * 0.5 + 1));
                            players[Besitzer[0]].ExpNow[Besitzer[1]] += punkte;
                            Kurzmeldung.Hinzufügen("+" + punkte.ToString(), players[Besitzer[0]].pos[Besitzer[1]], Color.Green, -300);
            ...
            else
            {
                ...
                for (int b = 0; b < Haeuser.Lebenspunkte.Count; b++)
                    if (Haus.HAEUSER_ZERSTOERUNG)
                    {
                        ...
                        if (found > 0)
                        {
                            ...
                            if (found > 0 && Waffendaten.Verschiessbar[Art] != 4) Kurzmeldung.Hinzufügen("-" + found.ToString(), new Vector2(Haeuser.Position[b].X + Haeuser.Bild[b].Width / 2 * Gebäudedaten.SKALIERUNG.Wert[id] + Gebäudedaten.POSITIONX.Wert[id], Haeuser.Position[b].Y + Gebäudedaten.POSITIONY.Wert[id] - Haeuser.Bild[b].Height * Gebäudedaten.SKALIERUNG.Wert[id]), Color.Yellow); //(Haeuser.Besitzer[b] == -1 ? Color.Yellow : players[Haeuser.Besitzer[b]].Farbe)
                        }
                    }
                    ...
                    for (int b = 0; b < players[i].TunnelAnlage.Count; b++)
                        if (Haus.HAEUSER_ZERSTOERUNG)
                        {
                            ...
                            if (found > 0)
                            {
                                ...
                                if (found > 0 && Waffendaten.Verschiessbar[Art] != 4) Kurzmeldung.Hinzufügen("-" + found.ToString(), new Vector2(players[i].TunnelAnlage[b].Position.X + Texturen.tunnel.Width / 2 * Tunnel.SKALIERUNG, players[i].TunnelAnlage[b].Position.Y - Texturen.tunnel.Height * Tunnel.SKALIERUNG), Color.Yellow); //(Haeuser.Besitzer[b] == -1 ? Color.Yellow : players[Haeuser.Besitzer[b]].Farbe)
                ...
                //if (Bunker.BUNKER_ZERSTOERUNG.Wert)
                {
                    ...
                    if (found > 0)
                    {
                        ...
                        if (found > 0 && Waffendaten.Verschiessbar[Art] != 4) Kurzmeldung.Hinzufügen("-" + found.ToString(), new Vector2(Bunker.Position[b].X + Texturen.bunker[0].Width / 2 * Optimierung.Skalierung(0.25f), Bunker.Position[b].Y - Texturen.bunker[0].Height * Optimierung.Skalierung(0.25f)), Color.Yellow); //(Haeuser.Besitzer[b] == -1 ? Color.Yellow : players[Haeuser.Besitzer[b]].Farbe)
                        ...
                        if (Bunker.Lebenspunkte[b] <= 0)
                        {
                            ...
                            Bunker.Entfernen(b);
                            Karte.Explosion_einer_Waffe_zünden(Spielfeld, gameTime, this, 0, po + new Vector2(Texturen.bunker[0].Width / 2 * Optimierung.Skalierung(0.25f), -Texturen.bunker[0].Height / 2 * Optimierung.Skalierung(0.25f)));
                        }
                    }
                }
            }
        ...
        /// <param name="screen">The screen.</param>
        /// <param name="symmetrisch">if set to <c>true</c> [symmetrisch].</param>
        unsafe public void Init(int Kartengroesse, Vector2 screen)  // Spiel Init -- Aufruf nicht nötig
        {
            ...
            Karte.create_map(Spielfeld, (int)((double)screen.Y * 0.75), (int)((double)screen.Y * 0.5), 30, 50, 50, (int)screen.Y, hoehlen);

            // Spieler erstellen
            players = new Spieler[2];
            for (int i = 0; i < players.Length; i++) { players[i] = new Spieler(); } // Initialenergie

            Missile = new Waffen[100];
            for (int i = 0; i < Missile.Length; i++) { Missile[i] = new Waffen(); Missile[i].ID = i; }
            InitialisiereSpieler(symmetrisch);

            Fenster.X = 0; Fenster.Y = 0;
        ...
        public void InitialisiereSpieler(bool symmetrisch)
        {
            ...
            Vector2[] Positionen = new Vector2[players.Length];
            ...
            if (symmetrisch)
            {
                PanzerTypen = new int[] { 0, 2, 3, 1 };
            }
            else
            {
                PanzerTypen = new int[] { 0, 1, 2, 3 };
            }
        ...
        public void Left(int player, int id) // bewegt den aktuellen Panzer nach links
        {
            //  if (Client.isRunning) return;
            if (id <= -1) return;
            if (Spieler.PLAYER_FUEL.Wert && players[player].Rucksack[id].GibTreibstoff() < Fahrzeugdaten.VERBRAUCH.Wert[players[CurrentPlayer].KindofTank[id]]) return;
        ...
        public void next_player() // setzte auf nächsten Spieler
        {
            ...
            if (SpielerAktiv() > 0)
            {
                ...
                for (; CurrentPlayer >= 0; i++)
                {
                    ...
                    for (int b = 0; b < players[i].pos.Count; b++)
                    {
                        if (players[i].isthere[b]) { check = true; break; }
                    }
                    if (check) CurrentPlayer--;
                }
                i--;
                Hauptfenster.Tausch.CurrentPlayer = i;
                CurrentPlayer = i;
                ...
                if (Spiel.CREDITS.Wert) players[CurrentPlayer].Credits += players[CurrentPlayer].Generate_Credits(Haeuser, CurrentPlayer);

                if (Spiel.TIMEOUT.Wert) Timeout = players[CurrentPlayer].MaxTimeout;
                if (Spiel.SCHUESSE.Wert) Schuesse = players[CurrentPlayer].MaxSchuesse;
                if (!Moving_Map) Set_Focus_X(players[CurrentPlayer].pos[players[CurrentPlayer].CurrentTank]);
            }
            else
                CurrentPlayer = -1;
        ...
        public bool PrüfeBau(Vector2 pos, int radius)
        {
            ...
            if (pos.X - radius / 2 > 0 && pos.X + radius / 2 < Spielfeld.Length)
            {
                ...
                {
                    breite = (int)(Texturen.haus[Haeuser.HausTyp[i]].Width * Gebäudedaten.SKALIERUNG.Wert[Haeuser.HausTyp[i]]);
                ...
                for (int i = 0; i < players.Count(); i++)
                    for (int b = 0; b < players[i].TunnelAnlage.Count; b++)
                    {
                        if (pos.X - radius / 2 <= players[i].TunnelAnlage[b].Position.X + breite && pos.X - radius / 2 >= players[i].TunnelAnlage[b].Position.X)
                        {
                            return false;
                        }
                        else
                            if (pos.X + radius / 2 >= players[i].TunnelAnlage[b].Position.X && pos.X + breite / 2 <= players[i].TunnelAnlage[b].Position.X + radius)
                            {
                                return false;
                            }
                    }
        ...
        public void SetPanzer(Spieler[] players, bool symmetrisch)
        {
            ...
            {
                int abstand = 150;
                int start = (int)(Spielfeld.Length / 4 - ((float)players[0].pos.Count / 2 * abstand));
                ...
                {
                    players[1].pos[i] = new Vector2(middle + (middle - players[0].pos[i].X), Spielfeld[(int)(players[0].pos[i].X)][0]);
                }
            }
            else
            {
                int[] dat = new int[Spielfeld.Length];
        ...
        public void UpdateMissles(GameTime gameTime, List<Karte.ParticleData> smokeList)  // geändert
        {
            for (int i = 0; i < Missile.Length; i++)
            {
                ...
                Vector2 notset = new Vector2(-99, -99);
                ...
                Karte.AddExplosion(smokeList, Missile[i].Last_Position[Missile[i].Last_Position.Length - 1], (int)Waffendaten.Daten3[Missile[i].Art].X, Waffendaten.Daten3[Missile[i].Art].Y, Waffendaten.Daten3[Missile[i].Art].Z, gameTime, Waffendaten.Farben[Missile[i].Art], Missile[i].Art, 1);
            }
        }
