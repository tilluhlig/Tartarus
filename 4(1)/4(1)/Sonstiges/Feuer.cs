...
namespace _4_1_
{
    ...
    {
        /// <summary>
        /// Liste der Brände (für jede x-Koordinate existiert eine Liste im array), y1, y2, zeit, altes Material an dieser Position
        /// </summary>
        ...
        public static List<Vector3> AlleBrandherdeVerkleinern()
        {
            ...
            for (int b = 0; b < Braende.Count(); b++)
            {
                ...
                for (int i = 0; i < Braende[b].Count; i++)
                {
                    ...
                    if (Braende[b][i].Z <= 0)
                    {
                        int W = (int)Braende[b][i].W;
                        int X = (int)Braende[b][i].X;
                        int Y = (int)Braende[b][i].Y;
