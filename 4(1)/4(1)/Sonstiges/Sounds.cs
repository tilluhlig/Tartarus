    ...
    internal static class Sounds
    {
        ...
        public static void Load(ContentManager Content) // lädt alle Sounds
        {
            ...
            for (int i = 0; i < Shots_Dateien.Count(); i++)
            {
                ...
                {
                    Shots[i] = new Soundsystem("Sounds\\" + Shots_Dateien[i] + Typ, Shots_volume[i], Shots_frequenz[i], false, Shots_initial[i]);
                }
            }
            ...
            for (int i = 0; i < Explosion_Dateien.Count(); i++)
            {
                ...
                {
                    Explosion[i] = new Soundsystem("Sounds\\" + Explosion_Dateien[i], Explosion_volume[i], Explosion_frequenz[i], false, Explosion_initial[i]);
                }
            }
            ...
            String[] Dateien = { "Sounds\\vehicle084", "Sounds\\vehicle077_2", "Sounds\\vehicle069", "Sounds\\vehicle121", "null", "null" };
            float[] Dateien_volume = { 0.6f, 0.7f, 0.6f, 1f, 0.0f, 0.0f };

            for (int i = 0; i < Dateien.Count(); i++)
            {
                if (Dateien[i] != "null")
                    PanzerMove[i] = new Soundsystem(Dateien[i] + Typ, Fahrzeugdaten.VOLUMES.Wert[i], Dateien_volume[i], true);
            }
            ...
            for (int i = 0; i < Panzer_begin.Count(); i++)
            {
                ...
                {
                    Panzer_rohr_begin[i] = new Soundsystem("Sounds\\" + Panzer_begin[i] + Typ, Panzer_begin_volume[i] * 0.65f, Panzer_begin_frequenz[i], false);
                    Panzer_rohr_end[i] = new Soundsystem("Sounds\\" + Panzer_end[i] + Typ, Panzer_end_volume[i] * 0.65f, Panzer_end_frequenz[i], false);
                    Panzer_rohr_loop[i] = new Soundsystem("Sounds\\" + Panzer_loop[i] + Typ, Panzer_loop_volume[i] * 0.65f, Panzer_loop_frequenz[i], true);
                }
            }
        }
    }
