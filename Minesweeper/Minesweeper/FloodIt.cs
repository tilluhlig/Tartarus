...
namespace Minesweeper
{
    public static class FloodIt
    {
        ...
        public static int Breite = 0; //wieviele Felder
        public static int Hoehe = 0; // wieviele Felder
        ...
        public static int[] Spielfeld; // 0 = aufgedeckt, 1-6 Farben
        private static bool[] fertsch;
        private static PictureBox[] Bilder; // die Bilder des Spielfelds
        private static PictureBox[] Farben; // die Bilder des Spielfelds
        ...
        private static void Berechnen(int x, int y)
        {
            ...
            if (Spielfeld[pos] == 0)
            {
                ...
                if (x > 0 && !fertsch[(x - 1) + y * Breite] && (Spielfeld[(x - 1) + y * Breite] == Farbe || Spielfeld[(x - 1) + (y) * Breite] == 0)) { int temp = (x - 1) + (y) * Breite; fertsch[temp] = true; Spielfeld[temp] = 0; Berechnen(x - 1, y); }
                if (x < Breite - 1 && !fertsch[(x + 1) + y * Breite] && (Spielfeld[(x + 1) + y * Breite] == Farbe || Spielfeld[(x + 1) + (y) * Breite] == 0)) { int temp = (x + 1) + (y) * Breite; fertsch[temp] = true; Spielfeld[temp] = 0; Berechnen(x + 1, y); }

                if (y > 0 && !fertsch[(x) + (y - 1) * Breite] && (Spielfeld[(x) + (y - 1) * Breite] == Farbe || Spielfeld[(x) + (y - 1) * Breite] == 0)) { int temp = (x) + (y - 1) * Breite; fertsch[temp] = true; Spielfeld[temp] = 0; Berechnen(x, y - 1); }
                if (y < Hoehe - 1 && !fertsch[(x) + (y + 1) * Breite] && (Spielfeld[(x) + (y + 1) * Breite] == Farbe || Spielfeld[(x) + (y + 1) * Breite] == 0)) { int temp = (x) + (y + 1) * Breite; fertsch[temp] = true; Spielfeld[temp] = 0; Berechnen(x, y + 1); }
            }
        }
        ...
        public static void InitSpielfeld(int _Breite, int _Hoehe, Form1 frm, PictureBox Zeichenflaeche, PictureBox Farbenflaeche, ImageList _Pictures, Label _Schrittanzeige, List<int> data)
        {
