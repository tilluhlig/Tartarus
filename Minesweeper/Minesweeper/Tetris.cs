...
namespace Minesweeper
{
    internal class Feld
    {
        ...
        public bool Bewegung = false; // false==fest, keine Bewegung    true==in Bewegung, fällt noch
    }

    internal class Tetris
    {
        private static int Zeilen = 0; // wieviel Zeit ist bereits verstrichen
        private static int Breite = 0; //wieviele Felder
        private static int Hoehe = 0; // wieviele Felder
        private static int Geschw = 500; // Geschw in millisekunden
        private static int Warte; // Steine bis geschw erhöht wird
        private static int nextStein = 0;
        ...
        private static Button A; // unwichtig
        private static ComboBox B; // unwichtig
        ...
        private static Feld[] Spielfeld; // speichert das Spielfeld
        private static PictureBox[] Bilder; // die Bilder des Spielfelds
        private static bool modus = false;
        ...
        private static void PrüfeNiederlage()
        {
            ...
            {
                if (Spielfeld[i].Art > 0 && !Spielfeld[i].Bewegung) { StopSpiel(); return; } // wenn ja, dann zerstöre das Spiel
            }
            ...
            else
            {
                ...
                for (int i = Hoehe - 1; i >= 0 && !found; i--)
                {
                    for (int b = 0; b < Breite && !found; b++)
                    {
                        ...
                        if (Spielfeld[pos].Art > 0 && Spielfeld[pos].Bewegung)
                        {
                            if (i == Hoehe - 1 || (Spielfeld[(i + 1) * Breite + b].Art > 0 && !Spielfeld[(i + 1) * Breite + b].Bewegung))
                            {
        ...
        private static void Verschiebe_Links()
        {
            ...
            for (int i = Hoehe - 2; i >= 0; i--)
            {
                ...
                {
                    int pos = i * Breite + b;
                    if (Spielfeld[pos].Art > 0 && Spielfeld[pos].Bewegung && (b == 0 || Spielfeld[i * Breite + b - 1].Art > 0 && !Spielfeld[i * Breite + b - 1].Bewegung))
                    {
                        return;
                    }
                }
        ...
        private static void Verschiebe_Rechts()
        {
            ...
            for (int i = Hoehe - 2; i >= 0; i--)
            {
                ...
                {
                    int pos = i * Breite + b;
                    if (Spielfeld[pos].Art > 0 && Spielfeld[pos].Bewegung && (b == Breite - 1 || Spielfeld[i * Breite + b + 1].Art > 0 && !Spielfeld[i * Breite + b + 1].Bewegung))
                    {
                        return;
                    }
                }
        ...
        private static void Drehe()
        {
            ...
            List<Point> Objekt = new List<Point>();
            List<Point> ObjektNeu = new List<Point>();
            int Farbe = 1;
            for (int i = Hoehe - 2; i >= 0; i--)
            {
                for (int b = Breite - 1; b >= 0; b--)
            ...
            Point Mitte = new Point((int)Math.Round((decimal)x / Objekt.Count, 0), (int)Math.Round((decimal)y / Objekt.Count));

            // Drehe Objekt
            for (int i = 0; i < Objekt.Count; i++)
            {
                Point Dist = new Point(Objekt[i].X - Mitte.X, Objekt[i].Y - Mitte.Y);
        ...
        private static bool DownVerändert = true; // damit man erst neu drücken muss, um die geschwindigkeit der steine zu erhöhen (Keys.Down)
            ...
            if (modus)
            {
                if (e.KeyCode == Keys.Left)
                {
                    ...
                    if (e.KeyCode == Keys.Right)
                    {
                        ...
                        if (e.KeyCode == Keys.Down && DownVerändert)
                        {
                            if (Geschw > 25) { Zeitgeber.Interval = 25; }
        ...
        public static void InitSpielfeld(int _Breite, int _Hoehe, Form1 frm, PictureBox Zeichenflaeche, ImageList _Pictures, Label _Zeilenanzeige, Button _A, ComboBox _B)
        {
            Pictures = _Pictures;
            ...
            for (int i = 0; i < Breite * Hoehe; i++)
            {
                ...
                Bilder[i].Top = ((int)(i / Breite) - 4) * 32 + Zeichenflaeche.Top;
